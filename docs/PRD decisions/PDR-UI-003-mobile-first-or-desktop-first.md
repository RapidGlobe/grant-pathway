...
The application must be fully responsive from 320px upward (C16, NFR-05). _⚠️ **This premise is superseded and is retained only as the context in which the decision below was taken.** The 320px requirement came from the Mark One BRD and was overridden by `ADR-ARCH-005`; `C16` and `NFR-05` were both corrected on 2026-08-05 (WJ's decision; `P5.0` register ref R-10) to state the real position — optimised at 1024px+, functional-but-unoptimised 768–1023px, **blocked below 768px**, not a mobile product in v1. Do not cite this sentence as a current requirement._ The primary persona Margaret uses a personal Windows laptop, and David uses a work laptop — both suggesting desktop as the primary device. However, volunteers and charity workers may access the app on mobile or tablet devices. Mobile-first development typically produces cleaner, more accessible responsive layouts because constraints force simplicity. Desktop-first is often faster when the primary use case involves significant text input, which grant writing does. This decision shapes the entire CSS and layout approach for the build.
