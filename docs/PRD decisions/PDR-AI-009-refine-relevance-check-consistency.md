...
**Follow-up finding, same day, 2026-07-17 (Stony Stratford Town Council testing) — false-positive isolated to compound multi-part questions, no change made.** WJ hit the warning repeatedly on §5 "Application Background" (5 sub-asks: need, how identified, user involvement, headcount, duration) and §8/9 "General Activities of the Group" (3 sub-asks: activities, meeting frequency, accessibility), raising whether the check needed loosening. Investigation found both test answers also reused an identical boilerplate opening sentence copy-pasted from a different question's answer — itself a plausible independent trigger ("boilerplate unrelated to the question" is explicitly named in the prompt as something that should fail the check) — so the compound-question structure alone couldn't be confirmed as the cause from those two cases. WJ then isolated it with a clean test: §11 "Publicity and Marketing" (a single, non-compound question) with a fresh, on-topic, non-reused answer — **no warning fired.** This confirms the relevance check works correctly for ordinary single-ask questions; the compound-question case (partial coverage of a multi-part ask, e.g. addressing 2 of 5 sub-points) is a distinct, narrower pattern than the original Henry Smith irrelevant-filler-text case this PDR was built to fix. **WJ's call: no change needed** — a compound question's guidance genuinely does ask for several distinct things, and flagging an answer that only covers some of them is a defensible reading of "does not appear to address the question," not clearly wrong the way silently passing wholly off-topic filler text was. Left as-is; revisit only if this recurs as a live-use complaint rather than a testing artefact.
