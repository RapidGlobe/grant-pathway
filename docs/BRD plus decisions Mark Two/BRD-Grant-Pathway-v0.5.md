...
| BD-03 | Non-narrative questions are pre-filled from profile where possible; shown as reminders otherwise | Data-entry questions are not a writing problem; AI should not be used on them                                                       | **Not built — see note below**                             |
| BD-04 | Question-level typing replaces funder-level type as the primary classification                   | Funder type (structured/narrative) determines output format only; question type determines how each field is handled                | **Partially built / terminology amended — see note below** |
...
**BD-03 note:** Never implemented. The AI extraction prompt discards non-narrative questions entirely rather than extracting them for pre-fill or reminder display. This surfaced during Clothworkers testing (`IT-CW-08`) and is currently an **open decision**, not a resolved gap: whether to build BD-03 as originally specified now depends on whether Grant Pathway is extending toward broader "any guideline or form" coverage (in which case BD-03 becomes more important, not less) or staying narrowly scoped to narrative writing assistance (in which case BD-03 and this BRD's Section 3.2/5.5 should be formally revised to describe the narrower scope as the real decision, not an unbuilt aspiration). See `docs/Test Plans/Clothworkers-Foundation-test-plan.md`, IT-CW-08, for the full trace.
