...
| G5  | Vercel        | **Which Function Regions are selected?** Dashboard-only, invisible to code review and CI, and Vercel's default for every new project is **Washington, D.C. (`iad1`)** — so this is wrong by default rather than by mistake. | ✅ **`GAP-110`, raised and closed 2026-08-17.** Production had `lhr1` **and** `iad1` selected; Axiom showed about half of all lambda execution in US regions. `iad1` unticked 2026-08-17, leaving **London only** — decision and reasoning in `ADR-STACK-004`. ✅ **Verified: `/api/health` returns `region: "lhr1"`** — the function reporting `VERCEL_REGION` about itself. ⚠️ **Do not use Axiom's `vercel.region` field for this check.** It showed `fra1` as the largest region while Frankfurt has never been ticked, so it is not reporting execution region — most likely the edge location that received the request. `/api/health` is the check. **Re-check after any Vercel project change, and check dev as well as prod — dev's regions have never been read.** |
