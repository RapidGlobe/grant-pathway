...
| G5  | Vercel        | **Which Function Regions are selected?** Dashboard-only, invisible to code review and CI, and Vercel's default for every new project is **Washington, D.C. (`iad1`)** — so this is wrong by default rather than by mistake. | 🔵 **`GAP-110`, raised 2026-08-17.** Production had `lhr1` **and** `iad1` selected; Axiom showed about half of all lambda execution in US regions. `iad1` unticked 2026-08-17, leaving **London only** — decision and reasoning in `ADR-STACK-004`. ⚠️ **Configured, not yet verified**, and **`fra1` (Frankfurt) appears in the logs despite never having been ticked**, which is unexplained. **Re-check after any Vercel project change, and check dev as well as prod — dev's regions have never been read.** |
