...
✅ **Proven on production, `GCM-01`, 2026-08-20:** the Idlewild fixture now extracts **16 questions, not 14**. The two mandatory date questions (source Q17, Q18) are back and typed `date`; the funding-amount question is typed `number` and `Budget`; and — the outcome that mattered most — the three prose budget questions (cost breakdown, grants raised, pending income) were **left `narrative`**. Over-classification was the real risk of this fix, and it did not happen. Rendering confirmed by screenshot: short boxes with placeholder hints, no counters, no AI assist, citations intact; Q11–13 still textareas with word counters.

**Why the estimate was wrong by an order of magnitude, which is the part worth keeping.** The deferral priced this behind the supporting-document contract change, at 3–5 weeks. It took a day and a half. Three things the estimate had not checked: `item_type` already contained `date` and `number`, so **no migration was needed**; the Word export selects only `item_order`, `item_label` and `answer_text`, so **a date flows through it untouched**; and `is_budget_question` was already a complete end-to-end precedent for a field travelling from the extraction prompt through Zod, the insert, the Step 4 renderer and the refine-answer guard — so the shape of the work was known rather than speculative. **The lesson is not "we were pessimistic"; it is that the estimate was made without reading the export path or the enum.**

**What the defect actually was.** Not a missing enum value, and not a rendering bug. `lib/prompts.ts` asked the model to extract only questions needing a NARRATIVE TEXT answer while simultaneously forbidding it to drop a question because its answer is a date (`GAP-39`). With no field in which to say "this is a date", the model had to resolve that contradiction itself, and did so differently on different documents: Radcliffe extracted its project dates as prose writing cards, Idlewild dropped its two mandatory date questions entirely. **Same rule, opposite outcomes** — which is why the fix is a new field rather than better wording.

**Two removals did the real work.** `"dates"` came out of the administrative data-entry exclusion list, and `"Date field dropdown"` came out of the table-format skip-list — that second entry is precisely what dropped Idlewild's Q17/Q18, and it had sat there since the skip-list was written, under an EXCEPTION clause that told the model to override it.

**Design decisions taken by WJ, 2026-08-20.** (1) A **short free-text box, not a date picker** — funders word these asks loosely and "April 2027" or "on receipt of funding" is frequently the answer the form invites; a calendar control would refuse it. A test pins this so it cannot be silently "improved" later. (2) **Number questions get the same treatment** as dates. (3) **AI assist suppressed**, reusing the budget suppression. (4) **A placeholder hint rather than £ formatting** on budget number questions, after `GCM-01`'s run showed a bare box next to the governance money fields — an AI-extracted budget question is not guaranteed to be a single figure, so the currency shape must not be forced.

**`FR-45` needs amending, not building.** The PRD's Question-Level Typing requirement says `date` questions should be "shown as read-only reminders only". **That has been deliberately rejected**, not deferred: a read-only reminder means a mandatory question the charity cannot answer, reaching the export blank — nearer to the `D-021` defect than to a fix for it. Recorded in §6.10 for revision on the next pass.

**Two structural details that would have caused silent faults.** `firstRefineButtonIndex` (which card gets `PDR-UI-008`'s AI-help tooltip) was `!isBudgetQuestion && fieldKey == null` — a condition a date question satisfies, so with a date first in the list the tooltip would have attached to a card showing no refine button. And `PDR-AI-012`'s combined word-limit filter used the same `fieldKey == null` test, so it would have counted a date towards a shared word total. The second is unreachable today (aggregate limits are free_form-only, and free_form sections are always narrative) and was changed anyway, because it is the kind of fault that surfaces years later as an unexplained word count.

⚠️ **Still owed before `D-021` can be closed — do not read the above as more than it says.** `RT-04` has NOT been re-run: the attempt on 2026-08-20 hit an eligibility hard stop after 1m 4s (a different charity profile from the 2026-08-19 run, so not a regression — the same fixture reached Step 4 that day) and never reached Step 4. **`GCM-06` and `GCM-07` are the regression risk and are untested**: §4e/§4f become `number` items and must still appear — they are `GAP-39`'s original two misses — and §5b must still yield four `Budget` cards with the £ prefix and Trim-to-limit unchanged. **No export has been checked with a date answer in it.** Prompt changes are behavioural: `GAP-39`, `GAP-40` and `GAP-51` were each rules that read correctly and then behaved differently live, so green unit tests are not evidence here.

⚠️ **A near-miss in the tests themselves, logged because it is the fourth instance of one pattern.** Six new prompt assertions failed on first run because `prompt()` inside the new `describe` block resolved to jsdom's global `window.prompt`, which returns `null`. The positive assertions failed loudly and exposed it — but the one **negative** assertion (`.not.toMatch` on the removed skip-list entry) **would have passed against `null`**, reporting the fix as verified while testing nothing. Same shape as `D-017`'s empty Sentry view, `RT-16`'s log query needing a control, and the load harness's phantom contamination pass: **an absent result read as a clean one.** Every negative assertion in the new Step 4 tests is now paired with a positive control — "no counter on a date" would otherwise pass just as well if the counter had vanished from all 16 cards.

---

## 2026-08-20 — the Schema Drift Check was red on both projects, and the cause is the exact inverse of the trap `RT-00` was strengthened to catch
...
### `GAP-108` — the Preview-scope worry is answered, and it is the good answer

A standing instruction in two documents said to check whether Preview scope's `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` hold **production** values — if they did, a working preview would read and write the production database. **Checked 2026-08-20 with a fresh `vercel env pull --environment=preview`: they are empty**, as are the anon key, `CRON_SECRET` and the AWS keys. **A preview cannot reach the production database because it cannot reach any database.**

This also **confirms the diagnosis rather than merely being consistent with it** — the variables `lib/env.ts` throws on at server startup are exactly the ones that are empty. ⚠️ **The gap stays open:** what changes is its urgency, not its size. It also re-frames option (1) — giving Preview its own dev-scoped credentials is plainly additive, since there is nothing to overwrite. **Recorded so the check is not run a third time;** an earlier snapshot of the same file from 2026-06-02 showed the same thing, and nobody knew.

⚠️ **This check cost a self-inflicted incident, recorded because the lesson is cheap and the repeat would not be.** The command that wrote this entry was a Python one-liner passed to `bash -c`, and the **backticks in its Markdown were expanded by the shell as command substitution**. One of them executed `vercel env pull --environment=preview`, which **overwrote `.env.local` in place** with Preview scope's values — emptying nine real local variables, the dev Supabase URL, anon key and service-role key among them. **Nothing recoverable was lost from the repository** (`.env.local` is correctly gitignored and was never committed) but **Vercel's Development scope is empty**, so the file was the only copy of those values and they had to be re-entered by hand. The two documented, non-secret values were restored from the repository — the dev project ref from `RT-00` and `AWS_REGION` from `technology-stack.md`. **The rule this yields: never pass prose containing backticks through `bash -c`; write it to a file and run the file.** The corrupted first version of this very section, with every backticked term silently eaten, is the other half of the evidence.

### Restoring `.env.local` found two faults in the values, and one in the harness's reporting

The nine variables emptied by the incident above were re-entered by hand, and **two pastes were silently malformed** — worth recording because neither produced an error message that named the cause.

1. **The Supabase anon key was truncated.** It decoded correctly, named the right project and the right role, and was rejected as `Invalid API key`. **The signature segment was 14 characters where a valid one is 43** — visible only by comparing it against the service-role key in the same file. A JWT with a valid header and payload looks entirely healthy to every check short of using it.
2. **The Upstash token was pasted across two lines**, leaving a space at the wrap. Every AI request then returned a bare `HTTP 500`, and the actual cause was one line in the dev server log — `Headers.append: "Bearer …" is an invalid header value`, thrown at the rate-limit call before any AWS credential was touched. **The visible symptom pointed at the AI provider; the fault was in the rate limiter.** Upstash's own client had also printed a warning naming the whitespace, which is the line worth reading first next time.

**A whole-file check is now the cheap way to catch both:** every value on one line, quotes closed, no internal whitespace, and — for the two Supabase JWTs — three dot-separated segments with a ~43-character signature.

⚠️ **The harness reported a pass over a check it had not performed, and that is the finding worth keeping.** With every request failing, it printed **"CROSS-CONTAMINATION: PASS — no summary contained another user's canary (2 checked)"**. It had checked nothing: the count came from the number of users fired rather than the number of summaries actually searched. **The single most important check in the harness — the one whose failure would mean one charity seeing another's material — was the one that could report success without running.** Fixed: it now reports **NOT TESTED** when no summary came back, and **PARTIAL** when some requests failed, with the count taken from summaries searched. Both paths were exercised against an unreachable port.
...
**`MK-Community-Foundation-test-plan.md` re-run against the June 2026 Oak Grants edition (MKCF-01–09 all Pass)**, closing the "not yet re-verified" status open since the guidelines were refreshed 2026-08-10. The 19-question upload-path count held exactly, independently re-derived from the source PDF rather than assumed. AI summary generation measured 43s for this 9-page PDF — within NFR-01's 45s large-document tier, comfortably inside `GAP-52`'s 6000-token cap (4110/6000 output tokens, clean `end_turn`). Two real defects surfaced during this run:

- **`GAP-90` — governance-fact extraction produced two entries with byte-identical text** when one guidelines sentence answers more than one of `PDR-AI-008`'s five fixed facts (e.g. "at least two unrelated signatories" answering both bank-signatory count and relatedness), visibly duplicating a Step 4 card. **Fixed the same day, with WJ's sign-off on the drafted wording:** `lib/prompts.ts`'s `governanceFacts` rule now instructs the model to write each entry's `questionText` toward that fact's own angle rather than reusing identical wording — citation reuse stays legitimate, since both facts genuinely share one source passage. Verified live against both known-affected funders before pushing, not just unit-tested: MKCF's bank-signatory facts now read distinctly, and Nationwide Building Society's guidelines (a harder case where one sentence combines _three_ facts — trustee count+relatedness, bank-signatory count, bank-signatory relatedness) produced three correctly distinct paraphrases.
- **`GAP-91` — clicking "Ready to assemble" flashed a false "We could not reach the server" error** on an otherwise-successful click, because `handleReadyToAssemble`'s `try/catch` in `application-step4-draft.tsx` didn't distinguish a genuine transport failure from the `redirect()` throw `setDraftReadyToAssemble` uses to signal success. Initially logged as "fires on every successful click" — **retracted the same day** when an identical click on the identical code path produced no false warning during a later retest; the true claim was "the code is unsafe and has been observed to misfire," not "always." **Fixed by decision, not left as an accepted quirk:** `unstable_rethrow` (`next/navigation`) is the officially documented API for exactly this case — the bundled Next.js docs name `redirect()` explicitly as an error that should be rethrown, not caught as an application error, so this was a correctness fix independent of how often it manifested. A scope check _before_ building found the original "likely shared by five other actions" claim was wrong — every other call site of the related redirect-ending actions already used the correct pattern (no catch at all, two with an explicit comment saying why, one via a native `<form action>`); only 2 catch blocks in 1 file, both calling `setDraftReadyToAssemble`, were actually at risk. Live-verified against the local dev server with no regression to the normal redirect path.

**Stony Stratford's revised application form retested** (the redesigned form WJ supplied, added to `docs/Grant Org Guidelines/` alongside the existing 2026 form rather than overwritten). Confirmed live: the AI's decision to split one shared-answer-box question (aims/timescales/people/other detail) into four separate numbered cards is intended behaviour, not a defect; the refine prompt correctly preserves line/paragraph structure per `GAP-46`'s existing rule while normalising incidental inter-word whitespace, which is not something that decision protects. This retest is also what surfaced `GAP-91` a second time (confirming the false-positive was real) and what let **`regression-test-plan.md`'s RT-09 caveat close** — WJ opened the real exported `.docx` in Word using genuinely multi-line, multi-paragraph answers this form's structure produces naturally, confirming `GAP-41`'s 2026-08-06 line-break fix in a real download for the first time since it shipped.

**`tnl-community-fund-application-form-2025.docx` deleted (WJ's decision).** Flagged as a suspected mismatch since the 2026-07-16 dashboard reset; confirmed this session as a personal application form for a board/committee-member appointment to TNL Community Fund's Wales Committee, not grant guidelines — the 0/5 governance-facts result on record was correct for what the document actually was. TNL/National Lottery Community Fund also isn't in `docs/target-funder-list.md`, so removed as out-of-scope clutter rather than replaced.

**`eligibility-check-test-plan.md`'s EL-02 re-run deliberately, closing a gap open since 2026-07-27.** The original result was a byproduct of AB Charitable Trust flagship testing: it confirmed 5 of 6 steps but never independently checked the dashboard "Ineligible" badge, and the fixture account was overwritten the same day, unable to reproduce even that. Re-run solo against a fresh throwaway account (Harry's Rainbow vs A B Charitable Trust, per the plan's own Test Data) rather than restoring the old fixture — all six steps confirmed, including the badge, genuinely red by computed style (`rgb(185, 28, 28)` on `rgb(254, 242, 242)`), application offering only Delete, no resume path.

**`GAP-52` pulled forward on WJ's request and moved to fully Built.** The truncation guard, the `response_too_long` error code, and the AI-slot refund had never actually fired since `SUMMARY_MAX_TOKENS` was raised to 6000 on 2026-08-07 — nothing in real usage had exceeded it, and `response-truncation.test.ts`'s 17 tests are a source scan (confirms the guard exists in the right shape), not a call to the route handler (confirms it executes correctly). Closed with a genuine integration test instead of waiting for a real document large enough to force a truncation: `__tests__/generate-summary-truncation-guard.test.ts` mocks Bedrock's response to report `stop_reason: 'max_tokens'`, calls the real `POST` handler with Supabase/rate-limit/Sentry all mocked, and confirms every step of the guard actually runs — `response_too_long` returned (not `parse_error`), exactly one Bedrock call (no doomed retry), `cancel_ai_slot` called correctly, Sentry tagged `ai_error: 'response_too_long'` — plus a control-case test proving the mocks aren't forcing the truncation branch on every input. The two related items `GAP-52` already flagged (`REFINE_MAX_TOKENS`'s missing guard, `ADR-AI-007`'s absent output-side policy) were raised formally the same day as `GAP-92` and `GAP-93` below.

**`GAP-92` built — `refine-answer` gains `GAP-52`'s truncation guard, and `GAP-94` fixed alongside it.** `app/api/refine-answer/route.ts` now checks `stop_reason === 'max_tokens'` before parsing, logs input/output tokens split, and returns a new `answer_too_long` `AiErrorCode` instead of falling into `parse_error` — deliberately not `response_too_long`'s guidelines-specific wording. `GAP-94` (found while investigating `GAP-92`): `handleRefine` was displaying the raw `AiErrorCode` string to the user instead of `aiErrorBody()`'s friendly message — now reads `data.message`, fixed in the same pass since the new truncation guard wouldn't have displayed correctly without it. New integration test `__tests__/refine-answer-truncation-guard.test.ts` (5 tests, mirroring `GAP-52`'s pattern); live-verified through a throwaway account that a genuine "Help me improve this" click still succeeds normally.

**`GAP-93` decided and built as new `ADR-AI-011-output-truncation-policy.md`, after WJ rejected the first proposed resolution.** A "500 words is too large" message for `refine-answer` was rightly rejected — many funders themselves cap answers at 500 words, so the real problem was `REFINE_MAX_TOKENS` set too low, not unclear wording. Real usage data (`ai_usage_log`/`application_items`, 151 `guideline_summary` + 170 `refine_answer` requests on `grant-pathway-dev`) was pulled before deciding: `generate-summary`'s combined token total showed no correlation with extracted-item count (input-document-length noise dominates the figure), so no defensible new ceiling could be set there; `refine-answer`'s real answers reached 630 words against an 800-token cap that left essentially no headroom before the JSON wrapper. Decision: `REFINE_MAX_TOKENS` raised 800 → 3,000 (Bedrock only generates tokens actually needed, so a higher ceiling costs nothing unless genuinely used); `ai_usage_log` gains persisted `input_token_count`/`output_token_count` columns (migration `20260814000000_gap93_output_token_tracking.sql`, `update_ai_slot_token_count` RPC extended with two optional params) so `SUMMARY_MAX_TOKENS` can be revisited from real data rather than guessed a second time in the same sitting; `SUMMARY_MAX_TOKENS` (6,000) deliberately left unchanged. Live-verified end-to-end through a throwaway account: a real 640-word answer refined successfully in one call using 697 of the new 3,000-token budget, and the resulting DB row's `input_token_count`+`output_token_count` summed exactly to the existing `token_count` for both AI routes. **`GAP-95` newly raised, found in passing while wiring this up:** `generate-summary`'s JSON-reprompt-parse-error retry has never added its own token usage into any total, combined or split — a pre-existing cost-accounting gap, not part of this decision.

**`GAP-95` built the same day it was raised.** `generate-summary`'s JSON-reprompt-parse-error retry (`retryResponse`) now adds its own token usage into `tokenCount`/`inputTokenCount`/`outputTokenCount` immediately after the retry succeeds, mirroring the existing `confirmResponse` pattern exactly. New regression test `__tests__/generate-summary-retry-token-accounting.test.ts` — deliberately verified to fail without the fix (reverted it locally, watched the test catch the shortfall, restored it) before being accepted as a real guard rather than a tautological pass.

`type-check`, `lint --max-warnings 0` and all 280 tests pass throughout (269 → 280 across today's session). `ADR-TRACEABILITY.md` → v2.73 (`GAP-90`–`95` all Built; register fully resolved, no open gaps). `TEST-DASHBOARD.md` → v2.65 (7 🟢, 0 🟡, 0 🔴). `MK-Community-Foundation-test-plan.md` → v2.6, `regression-test-plan.md` → v2.15, `help-and-tooltips-test-plan.md` → v2.4, `eligibility-check-test-plan.md` → v1.4.

---

## 2026-08-13 (latest) — `GAP-05` built: the below-768px mobile viewport banner — `accessibility-test-plan.md` moves 🔴 → 🟢 for the first time, P5.3 complete

`GAP-05` (`ADR-ARCH-005`'s below-768px blocking-banner consequence, tracked 🟡 Tasked only since 2026-06-16) is now built — the last open item barring `accessibility-test-plan.md` from a 🟢 mark. All 15 accessibility cases already Passed individually; AC-11 and AC-15 had each carved a small-viewport sub-check out of scope specifically because this banner didn't exist, and the plan's own "Known dependency" note barred 🟢 until it shipped.

**Built CSS-only, no JavaScript.** `components/mobile-viewport-banner.tsx` is a plain server component (`flex md:hidden`), wired into `app/layout.tsx` alongside a `hidden md:flex md:flex-1 md:flex-col` wrapper around the rest of the app. Tailwind's default `md` breakpoint is exactly 768px in this project (no custom `--breakpoint` override), so this maps directly onto the ADR's threshold with no custom media query, no `matchMedia`, and no hydration-mismatch risk — `display:none` removes the hidden side from paint and the accessibility tree without unmounting anything.

**Copy and icon decided fresh** (`docs/Business Design/DDR-CS-007-mobile-viewport-banner.md`, new — no prior spec existed for either): heading "Please use a desktop or laptop," body names a concrete reason (Step 4's side-by-side guidelines/draft layout) rather than a bare refusal, per the tone-and-voice guide's Plain English and Encouraging pillars. `MonitorSmartphone` (`lucide-react`) chosen as the icon — a clean first use, no existing usage to conflict with. A bypass-link option was considered and rejected outright: it would silently reopen the exact gap `ADR-ARCH-005` closed by deciding to block, not warn.

**Live-verified** at the exact 768px boundary, on both a public route and mid-application on an authenticated Step 4 page: banner shown and app `display:none` at 767px and 375px; banner hidden and app fully interactive at 768px, 800px and 1280px, with no visual regression to the existing header/footer flex stacking. **Dialog-portal edge case checked, deliberately not fully hardened:** Base UI's `Dialog` portals to `document.body` outside the new wrapper, so an auto-opening session-timeout modal at a narrow viewport isn't hidden by `display:none` — `z-[9999]` (vs the dialog's `z-50`) was verified live, via a simulated portaled overlay, to keep the banner the topmost element at screen centre regardless. A keyboard-only user below 768px could in principle still tab into a hidden dialog's controls without seeing them; left as documented residual risk rather than adding `inert`/focus-trap JavaScript, since sub-768px keyboard/AT testing is already out of scope (`accessibility-test-plan.md` line 46) and `GAP-05` is Low severity precisely because "block the UI" is the decided mitigation, not a fully-hardened one.

**AC-11's and AC-15's deferred sub-checks are closed, re-scoped rather than literally rerun.** Below 768px there is no longer an app to check — the banner has no small interactive targets and no text to measure — so the substantive re-verification was the first live sweep of the **768–1023px functional band**, which neither case had actually tested before (AC-11 measured only 1280px; AC-15's zoom proxy always landed at 640px, already sub-768 even before this banner existed). Ran a fresh throwaway account through the dashboard, charity profile, and a full Step 1–4 application at 800×900: target sizes (footer links, "Show password" button) measured unchanged from the 1280px readings; no elements exceeded the viewport's right edge on Step 3/4 (`GAP-78`'s reflow fix holds in-band); the only sub-12px text found was the same already-accepted `aria-hidden` decorative pattern (`GAP-57`'s precedent); line-height measured 1.5, unchanged. 3 new component tests (`__tests__/mobile-viewport-banner.test.tsx`); `type-check`, `lint --max-warnings 0` and all 269 tests pass.
...
**Ahead of the plan's own six steps**, testing Step 2's file upload surfaced two gaps. **`GAP-84`:** the "Uploading…"/"Processing document…" status text carried no live-region role at all — the same shape as `GAP-68`, on a screen not yet swept for it; fixed with `role="status"`. "Uploading…" was confirmed announced live across two separate test runs; "Processing document…" was not independently confirmed, since small test files process too quickly to catch it — the same caveat `GAP-68` itself carried for one of its three stages, not treated as a defect. **`GAP-85`:** selecting a file orphaned focus for a moment, with NVDA announcing "unknown" — the dropzone (the focused element when the file picker opened) unmounts the instant `uploadState` changes, dropping focus with nowhere defined to land. **Fixed same session, WJ's explicit choice** to address it immediately rather than defer: a persistent wrapper (`uploadRegionRef`, `tabIndex={-1}`) now receives focus synchronously before the dropzone unmounts. Live-verified twice, across two separate test sessions, with no "unknown" in either.

**Testing the "Guidelines loaded from this session" banner then confirmed `GAP-71`'s own deferred suspicion was correct.** `GAP-71` (found 2026-08-12) had fixed two conditional-mount `role="status"` banners but explicitly named `application-step2-form.tsx`'s remaining two (upload-success, guidelines-restored) as "likely sharing this failure... never actually confirmed working, only structurally checked" — and deferred the check to this case rather than assume either outcome. They were confirmed silent: the banner rendered correctly on screen (confirmed by screenshot) but nothing appeared in the Speech Viewer log. **`GAP-86`** fixes both by merging them into one permanently-mounted container (`sr-only` when inactive), matching `GAP-71`'s own proven shape. Live-verified: "Guidelines loaded from this session — Remove guidelines and upload a different document" now announces on arrival at Step 2.

**The plan's own steps then surfaced three more, each a genuinely new failure shape, not a repeat of `GAP-71`'s conditional-mount cause:**

**`GAP-87` (autosave "Saved" confirmation) — a design trade-off, not a mechanical bug.** The confirmation was structurally present and correctly coded, but WJ never heard it: `role="status"` (polite) was losing the race against the user's own next action within the confirmation's 2.5-second display window (`SAVED_INDICATOR_MS`) — a background 60-second dirty-field sweep or an on-blur save could fire while NVDA had no quiet moment to speak it. The non-interrupting behaviour had been a deliberate earlier decision, recorded in-file ("this must not interrupt... a success is something to notice and forget"), which this finding put directly in tension with actually being heard. **Escalated to WJ rather than fixed unilaterally**, since reversing a recorded design decision is not a mechanical fix: WJ chose to switch to `role="alert"`, trading the "must not interrupt" goal for reliability. Live-verified: WJ retyped an answer, blurred, and heard "Saved" clearly.

**`GAP-88` (offline "Not saved" banner) — silent despite already using `role="alert"`.** Forcing DevTools Offline and blurring a dirty field showed the banner correctly on screen but nothing in the Speech Viewer log — genuinely new, since the code already does what `GAP-87`'s fix reaches for. A second, unrelated `role="alert"` (Step 1's required-field validation error) was tested immediately after and did announce correctly, ruling out a blanket `role="alert"` failure across the app before assuming one. Fixed with the same always-mounted technique already proven for `role="status"` elsewhere in this file, which was sufficient on its own — the more invasive alternative (moving focus onto the banner, which would disrupt active typing for every user, not just screen-reader users) was named but not needed.

**`GAP-89` (progress count) — the `role="progressbar"` was a red herring.** Approving a question conveyed nothing about the overall approved count to NVDA. The `role="progressbar"` itself is correctly built (`aria-valuenow`/`valuemin`/`valuemax`/`aria-label`), but a progressbar's value change is not announced by NVDA unless the element has focus, which it never does here. The actual gap was the plain-text "X of Y approved" span beside it — what a sighted user actually reads to track progress — which carried no live-region role at all. Fixed with `role="status"`, the reliable shape since the span is already permanently mounted. Live-verified: "2 of 15 questions approved" announced alongside the already-working "Answer approved" confirmation.

Two distinct inline validation errors (Step 1's funder-name and grant-name required-field messages) and every success/confirmation message tested announced correctly, once each — no fix needed for either.

`type-check` passes throughout; no new automated tests — every fix is markup/role restructuring, the same class of change `GAP-67`–`GAP-71` already established. Documented in `ADR-TRACEABILITY.md` (`GAP-84`–`GAP-89` added, 89 rows, v2.59), `accessibility-test-plan.md` (AC-09 → Pass, v1.34) and `IMPLEMENTATION-STATUS.md`. **AC-01 through AC-09 now all Pass** — only AC-15 remains, blocked on `GAP-79`'s already-decided `px`→`rem` conversion, deferred to a later session.
...
**`GAP-80`:** a native form control (the charity-lookup input) wrapped in a stable `<span>` via `ContextualTooltip`'s `wrapInStableSpan` — required because a bare Base UI `Tooltip.Trigger` forces `type`/`name` props onto a polymorphic target, and setting `type` on a real `<input>` reinitialises it and blurs it instantly (`GAP-36`). The span carries the `aria-describedby`, but real DOM/NVDA focus lands on the `<input>` inside it, so the description was never read. **`GAP-81`:** a disabled button (the "Ready to assemble" control) in the same span shape, this time with `active={false}` making the span itself the genuine focus target (`tabIndex=0`) — yet NVDA reported the nested disabled button's own role/state and never the span's own description, an accessibility-tree quirk around a focusable role-less wrapper with a single disabled descendant. **`GAP-82`:** a bare role-less `<div>`/`<span>`/`<p>` (the budget-warning hint) used directly as the tooltip trigger, no span wrapper at all — NVDA reliably read the element's own visible text (name-from-content) but never its `aria-describedby`, since a role-less element carries no exposed "description" distinct from name. An explicit `role="group"` was tried live first and produced no change — ruled out by direct test, not assumed — before the actual fix.

`GAP-80`/`GAP-81` fixed together in `components/contextual-tooltip.tsx`: the shared tooltip id is now generated in `ContextualTooltip` itself via `React.useId()` (moved above the `active === false` early return to satisfy the rules of hooks) rather than solely inside `components/ui/tooltip.tsx`'s `Tooltip`, which now accepts an optional `id` override — this makes the id available to clone onto the actual descendant NVDA ends up describing, not just the span. `GAP-82` fixed separately and differently: rather than relying on `aria-describedby` at all for role-less triggers, the tooltip content is appended as an additional visually-hidden (`sr-only`) child node, folding it into name-from-content — the mechanism NVDA already reads correctly for these tags regardless of role. Each fix live-verified by WJ before moving to the next; `type-check`/`lint --max-warnings 0`/all 266 tests pass throughout.

Documented in `ADR-TRACEABILITY.md` (`GAP-80`–`GAP-82` added, 82 rows, v2.58), `accessibility-test-plan.md` (AC-08 → Pass, v1.32), `TEST-DASHBOARD.md` (v2.54) and `IMPLEMENTATION-STATUS.md`. **AC-01 through AC-08 now all Pass** — 2 of 15 accessibility cases (AC-09, AC-15) remain unstarted.
...
Set up a fourth fresh throwaway Supabase test account (`ac15-test@example.com`) and ran all four steps live rather than by inspection. **Step 1 (200% zoom):** proxied as a 640px viewport against the plan's own 1280px desktop baseline, since this sandbox cannot drive real browser zoom. `/`, `/profile` and Step 3 were clean immediately; Step 4 was not — see `GAP-78` below. **Step 2 (nothing under 12px):** found `GAP-77`, eleven `text-[11px]` instances one pixel under the documented floor, across `site-footer.tsx`, `dashboard-empty.tsx`, `application-step4-draft.tsx` (five) and `application-step5-approve.tsx`. Fixed the eight non-decorative instances to `text-[12px]`; left three `aria-hidden` decorative duplicates alone, the same precedent `GAP-57` already established. **Step 3 (1.5 line height):** passes as built (16/24px and 14/21px, both exactly 1.5) despite a minor undershoot against the design token's own documented 1.6 for "Body (small)" — an internal deviation, not a WCAG failure.

Found and fixed one more genuine bug outside the plan's four numbered steps, while live-testing Step 1 at 640px: `GAP-78`, a real SC 1.4.10 reflow failure in Step 4's question-card header. `CITATION_LABEL_MAX_LENGTH = 90` combined with a heading-path join had produced an abnormally long citation label (this test session's guideline text had a sentence misclassified as a heading), rendering a single-line button up to 630px wide that overflowed the 640px viewport even after the header row itself could wrap. Fixed with `flex-wrap` at two nesting levels plus `min-w-0`/`max-w-full`/`truncate` on the citation button, live-confirmed clean at 640px with no regression at 1280px. Also wrote and approved all three sections of the same test application through to Step 5 (Approve & Export) specifically to check its reflow, since it renders the same source badges `GAP-77` touched — clean at both widths, no fix needed (that badge's render branch is legacy-application-only and dead code for this application).

**Step 4 (browser default font size, not zoom) failed, exactly as predicted.** A real OS/browser font-size preference can't be driven from this sandbox, so this was proxied the way the mechanism actually works: browser font-size preferences resize the root element's `rem` base, not the viewport, so `html { font-size: 32px }` was injected directly (doubling the 16px default) and every previously-sampled text node on the live Draft Answers page was re-measured. Every one reported an identical computed `font-size` before and after — zero response. Logged as `GAP-79`: 384 `text-[Npx]` font-size instances across 30 files (no `leading-[Npx]` line-height instances exist, so the scope is font-size only), concentrated in `application-step4-draft.tsx` (69), `application-step3-summary.tsx` (35) and `charity-profile-form.tsx` (31). Per the plan's explicit instruction, this was recorded as a finding with a scope estimate rather than fixed inside the test run — it needs its own decision (convert to `rem`, a large but mechanical and low-risk-per-instance change, or amend §8.6 to accept `px` plus zoom as sufficient for AA).

`GAP-79` alone carries the overall result to Fail; `GAP-77` and `GAP-78` were both found and fixed the same day and do not themselves block it. This is a deliberate application of `ADR-OPS-006` ("accessibility violations are treated as bugs, not nice-to-haves") and the Defect Log rule that a deferred finding needs an explicit decision, not silence — recording Fail rather than a qualified Pass keeps that decision visible rather than buried in a Notes paragraph.
...
Two genuine WCAG 2.5.8 (Target Size, Minimum) failures were found and fixed. `GAP-75`: the shared `site-footer.tsx`'s three legal links ("Help centre", "Privacy policy", "Terms of service") are bare inline `<a>` elements with no padding, measuring only 16px tall — under the 24px minimum, and not eligible for the "inline text" exception since they sit inside a `<nav><ul>` list, not running prose. Because the footer renders on every page, this was the widest-reaching finding in the whole accessibility pass so far. Fixed by adding `inline-block py-1.5` to all three links, bringing them to a live-verified 31.5px. `GAP-76`: Step 4's "Keep my original" link, next to "Use this improved version" in the AI-suggestion panel, failed on two fronts at once — 19.5px tall on its own (under 24px) and only 16px from its neighbour (under the 24px spacing exception too). Fixed by widening the `flex` wrapper's gap from `gap-4` to `gap-6`, bringing the edge-to-edge distance to exactly 24px — satisfying the spacing exception without needing to grow the link's own hit box, consistent with how other isolated small text-action links in the app already pass.

Several other small targets were checked and confirmed to pass cleanly via the spacing exception rather than needing a fix: the shared modal close button (`size="icon-sm"`, 28×28px, confirmed via source since its size is fixed by a Tailwind class with no dynamic content), the sign-in page's "Show password" button (28×28px), Step 4's "Help me improve this" and "Trim to limit" buttons (each isolated by well over 24px from the next real target), and the citation badge. The citation badge could not be triggered live in this session — the AI only attaches a citation when the source guidelines have extractable page/heading structure, and this session's pasted guidelines were unstructured prose — so its size was inferred from its shared `px-2 py-0.5 text-[11px]` padding class, identical to sibling badges measured live at 20.5px tall on the same page; this is flagged in the test plan as an inference rather than a live reading, to be spot-checked next time real structured guidelines are used. A bullet documenting the target-size requirement and its exception was added to `design-requirements.md` §8.3. The throwaway test account and its one test application were deleted via the admin API at the end of the session.

## 2026-08-12 (later) — AC-10 (colour contrast) run solo and marked Pass; one new GAP found and fixed live; a sandbox tooling artefact ruled out

With AC-07 closed and WJ's NVDA involvement no longer needed, AC-10 through AC-15 were handed off to run solo. AC-10 went first: a fresh throwaway Supabase test account was created via the admin API (the established pattern for this sandbox, since the shared dev database has no reusable confirmed account and UI registration requires real email confirmation), and a genuine application was walked through Steps 1–4 — a real pasted funder-guidelines document, a real AI summary, and real content typed into the Draft Answers fields — rather than testing against empty states alone. `window.axe.run(document, { runOnly: ['color-contrast'] })` was driven directly at each step, the same axe instance AC-01 already uses, and deliberately-provoked edge states were added on top: an empty-form validation-error submission on the profile page, an over-word-limit answer (deliberately typed past 300 words to trigger the red over-limit styling), and the AI "Help me improve this" refinement panel.

That refinement panel surfaced `GAP-74`: its "Keep my original" link uses `text-[#64748B]`, the same slate already verified passing at 4.5:1+ against the several backgrounds it was checked on when `DEF-01`/`GAP-56`/`GAP-72` retired the too-light `#94A3B8`. But this panel's own background, `bg-[#EFF6FF]` (a light blue introduced for the AI-suggestion feature, not one of the backgrounds `#64748B` had been checked against), is different enough to drop the ratio to 4.37:1 — just under the 4.5:1 floor. Fixed by reusing `GAP-72`'s already-established replacement, `#475569` (6.96:1 on this background), rather than treating it as a new colour decision. Re-verified live: 0 `color-contrast` violations on the panel after the fix.

One near-defect surfaced and was ruled out rather than logged: axe flagged Step 2's "Continue" button at 2.23:1 immediately after pasting guidelines text into the textarea. The button's own `disabled` property and `:disabled` CSS match were both confirmed `false` — it was genuinely enabled, not a stale-state bug — yet its computed `opacity` remained stuck at `0.5`, the button's `disabled:opacity-50` resting value. The cause: the sandboxed Browser pane does not composite frames while not actively displayed, so CSS `transition`s that are mid-flight when a script reads them never get to finish — the button's disabled→enabled opacity fade froze partway through rather than completing. Confirmed by injecting `*, *::before, *::after { transition: none !important; animation: none !important; }` globally, after which the same button's opacity snapped cleanly to `1` and the axe violation disappeared. The same transition-neutralising style was re-injected on every subsequent page for the rest of the run to avoid repeating this false positive — a sandbox limitation, not a product defect, and not logged as a GAP.

The three large-text-only documented pairs, the red error/warning text, the muted greys, disabled-control exemptions, and the "no status by colour alone" requirement were all checked and held. One previously-undocumented but now-confirmed pair was added to `design-requirements.md` §8.1's table for completeness: `#B45309` on `#EFF6FF` (4.62:1), the AI-suggestion panel's own amber advisory text. The throwaway test account and its one test application were deleted via the admin API at the end of the session.

## 2026-08-12 — AC-07 (NVDA, five-step flow) completed and marked Pass; four new GAPs found and fixed live; a reproducible prettier bug worked around

WJ resumed driving NVDA live for Steps 4 through 8, closing out AC-07 — the last of the case's eight steps, following two earlier sessions that got through Steps 1–3. Step 4 (step indicator) found `GAP-70`: NVDA correctly announced "current step" via the existing `aria-current="step"`, but the `sr-only` text never stated step position or total, against `design-requirements.md` §8.4's own documented format ("Step 1 of 5, Application details, completed"). Fixed and live-verified via `NVDA+Ctrl+F` jumping straight to the step-4 item. A genuine NVDA Elements List quirk was investigated and ruled out as a tooling artefact rather than an app bug along the way: its Landmarks view failed to show the step indicator's nested `Application progress` landmark even after a hard reload, while `D`-key quick-nav found it correctly and raw `view-source:` HTML confirmed the markup is present and unhidden.

Steps 5 and 6 (citation links, answer field labels) both held cleanly with no findings. Step 7 (approval announcement) surfaced the session's most significant finding: approving a Draft Answers section produced the correct visual "Answer approved" banner but **zero speech** — confirmed live via screenshot that the approval had genuinely gone through, ruling out a click-registration failure before looking further. The root cause, logged as `GAP-71`: `GAP-67`'s original fix (2026-08-10) added `role="status"` to four confirmation banners, but two of them — Step 4's and Step 5's approval banners — are only _mounted_ into the DOM once a condition becomes true, and a `role="status"` element built that way is not reliably announced even with the role present the instant it appears. This is distinguishable from `GAP-68`'s loading-message fix, which already worked because it keeps one element permanently mounted and only changes its text. Fixed for the two confirmed sites by restructuring them the same way; `application-step2-form.tsx`'s two remaining `GAP-67` sites (upload-success, guidelines-restored) share the original conditional-mount pattern and are very likely equally broken, but were deliberately not fixed speculatively — deferred to `AC-09`'s own live-region sweep, tracked as an open task rather than silently assumed fine, since that file's upload flow is a multi-state machine deserving its own careful pass. Step 8 (export) held once `GAP-71` was fixed: download and completion both announced correctly, and the re-export confirmation dialog read its stored date cleanly.

Two live `axe-core` console findings, unrelated to NVDA, were also fixed the same session: `GAP-72` — two more instances of `GAP-56`'s already-diagnosed `#64748B`-on-`#F1F5F9` contrast failure, on two Draft Answers badges ("Counts toward X-word total", "Added by you") that are both conditional and had never been visible in any prior test run; completed `GAP-56`'s already-made rollout to `#475569` rather than raising it as a new design decision. `GAP-73` — Step 5's "Last exported" timestamp, `#16A34A` on `#F0FDF4` (3.14:1), a pairing never previously checked; matched to the passing `#166534` already used one line above it in the same banner, for the same reason.

**A reproducible prettier 3.9.6 bug was found and worked around while documenting the above.** Adding the four new GAP rows to `ADR-TRACEABILITY.md`'s Gaps register and running `prettier --write` corrupted the entire table — truncated words, dropped punctuation, duplicated markup — once the new content pushed any cell size near the table's existing longest row (~6,000+ characters); the corruption reached rows far from the edit, not just the new ones. Isolated and confirmed on a scratch copy before it could touch the committed file. WJ's decision: add `docs/Implementation Plan/ADR-TRACEABILITY.md` to `.prettierignore` rather than chase the exact trigger further today; `prettier --check` (read-only) is unaffected and remains safe.

`type-check` passes throughout; `ADR-TRACEABILITY.md` v2.53, `accessibility-test-plan.md` v1.24, `TEST-DASHBOARD.md` v2.46 all updated. **AC-01 through AC-07 now all Pass** — 8 of 15 accessibility cases (AC-08–AC-15) remain to run.

## 2026-08-12 — Dependabot alert #40 dismissed as inaccurate; PR #99's lint failure fixed without touching D-HSF-03's hard-navigation fix

WJ couldn't open `github.com/RapidGlobe/grant-pathway/security/dependabot/40` directly and asked whether the underlying `nanoid` advisory (GHSA-2v37-7h3g-55p8, CVE-2026-67213, infinite loop on `customAlphabet`/`customRandom` with `size: 0`) was still current. `npm ls nanoid` and a direct `package-lock.json` read confirmed both resolved copies — `postcss` → `nanoid@3.3.18`, `docx` → `nanoid@5.1.16` — already sit past the patched versions (3.3.17 / 5.1.6). The alert was open only because Dependabot hadn't re-evaluated it against the current lockfile. Dismissed via `gh api` with reason `inaccurate`.

Reviewing the two open Dependabot PRs at the same time: #100 (production-minor-patch group — `next` 16.2.12→16.3.0, `@anthropic-ai/sdk`, `@base-ui/react`, `@supabase/supabase-js`, `@upstash/redis`, `lucide-react`, `shadcn`, `bedrock-sdk`) passed all CI checks cleanly. #99 (development-minor-patch group) was failing `lint-and-typecheck`: `eslint-config-next` 16.2.12→16.3.0 ships a new rule, `@next/next/no-location-assign-relative-destination`, which flagged two pre-existing `window.location.href` calls under `eslint --max-warnings 0`.

The two call sites were not equivalent. `nav-authenticated.tsx`'s post-sign-out `window.location.href = '/'` had no documented reason for a hard navigation, and `session-timeout-provider.tsx` already runs the identical `signOut()`-then-navigate sequence via `router.push('/?timeout=true')` without issue — so it was switched to `router.push('/')` for consistency. `application-step4-prep-checklist.tsx`'s `window.location.href` is different: it is the **D-HSF-03** fix (this log, 2026-06-07) for a Router Cache staleness bug where Step 4 rendered "No specific questions found" after a soft navigation. Replacing it with `router.push()` would have silently reintroduced that bug, so it was kept as a hard navigation with a scoped `eslint-disable-next-line` and a comment pointing back to D-HSF-03, rather than "fixed" to match the new lint rule.

Verified locally against the bumped `eslint-config-next` (`npm ci` on the PR branch, then `npm run lint` / `npm run type-check`) before pushing to `dependabot/npm_and_yarn/development-minor-patch-846ee2f29c`. All five CI checks (lint-and-typecheck, test, validate-migrations, Vercel, Vercel Preview Comments) now pass on PR #99. Neither PR has been merged yet — that decision was left with WJ.
...
Steps 1–2 passed: the sign-in page (`/`) had zero headings, an isolated defect no prior AC case had exercised on this specific page — fixed with an `<h1>Sign in</h1>` matching the Register page's own established pattern. A second, unrelated `<h2>`→`<h1>` inconsistency was found on the same sweep, on the rare Step 3 eligibility-mismatch hard-stop screen. Landmark cycling (`D`/`Shift+D`) and the Elements List (`NVDA+F7`, reached via `CapsLock+Fn+F7` once the laptop's hardware F-key remapping was diagnosed) both confirmed clean landmark structure.

Step 3 (form field labels) is partway through, with two real findings: `GAP-65` — required fields are never announced as required anywhere in the app, including the two forms (Step 1, charity profile) that already show a visual asterisk, because that asterisk is `aria-hidden` and neither form ever added `aria-required`. A follow-up sweep of every form component found the gap was systemic — six more forms (Register, Sign in, Forgot password, Reset password, Account settings, Delete account) had no required-field indication at all, visual or accessible. `GAP-66` — the charity-lookup field's own label was announced twice; traced to `contextual-tooltip.tsx`'s `wrapInStableSpan` giving its wrapper `<span>` an unconditional `tabIndex={0}` even though the native input it wraps is already focusable, producing a redundant tab stop whose computed accessible name echoed the input's own `aria-label`. Fixed by making the span's `tabIndex` conditional on the _other_ case this same prop covers — a disabled Button that genuinely does drop out of the tab order and needs the span as its only keyboard-reachable tooltip trigger. A third observation, the disabled "Look up charity" button being unreachable until the search field has text, was confirmed as expected `disabled`-attribute behaviour, not a defect.

WJ then stepped away mid-session. While away, an independent structural read of Steps 2/4/5's source (role and `aria-live` wiring, since no authenticated browser session was available this session — typed input stopped reaching the sign-in form's React state, and the Browser pane was not actively compositing, a new tooling gap distinct from the file-upload and audio-transcription ones already on record) found `GAP-67`: four confirmation banners (Step 4's "Answer approved," Step 5's approval banner and re-open error, Step 2's upload-success/guidelines-restored banners) were missing the `role="status"`/`role="alert"` that each of their own files already uses for the identical kind of feedback elsewhere.

All three GAPs were fixed the same day. Verification is split, not uniform: the public-form half of `GAP-65` (Sign in, Register) was confirmed live via `axe-core`; everything touching an authenticated page — the rest of `GAP-65`, and all of `GAP-66`/`GAP-67` — is verified only via `type-check`, `lint --max-warnings 0` and `prettier --check`, with WJ's live NVDA re-confirmation the explicit remaining step. `application-step4-draft.tsx`'s dynamic per-question required/optional wording and `application-step5-approve.tsx`'s review checklist were deliberately left out of the required-field fix — their semantics are already conveyed through visible sentence text or a disabled-button gate, not the label-suffix convention this fix applies elsewhere. AC-07 remains **Blocked**: Steps 4–8 and the pending live re-confirmation are still open. Full detail in `ADR-TRACEABILITY.md` v2.50 and `accessibility-test-plan.md`'s AC-07 write-up.
...
Steps 1, 3, 4 and 5 held cleanly: the skip link is `sr-only` until focused and became visible on the first Tab on both layouts; nav and footer tab order ran top-left to bottom-right on both layouts, confirmed by a scripted Tab-and-record walk rather than by eye; every Help/Help centre/Privacy/Terms of service link's accessible name was confirmed — via both DOM `textContent` and the live accessibility tree — to include its visually hidden "(opens in a new tab)" text; and the step indicator's `<nav aria-label="Application progress">` landmark was confirmed reachable via landmark navigation with its current step discoverable via a "Current: …" `sr-only` prefix, while correctly carrying zero Tab stops itself (it's a progress display, not a control — WCAG best practice keeps it out of the Tab order rather than adding noise to it).

Step 2 — "activate the skip link and confirm focus lands inside `<main id="main-content">`" — did not hold literally, and became **`GAP-58`**. Neither `app/(public)/layout.tsx` nor `app/(authenticated)/layout.tsx` gave `<main>` a `tabindex`, so it isn't a valid focus target: activating the skip link scrolled the page and updated the URL fragment, but `document.activeElement` fell back to `<body>`, confirmed by direct DOM inspection rather than assumed. The practical effect was still correct for sighted keyboard users — Chromium's own "sequential focus navigation starting point" fallback still put the very next Tab press on the first focusable element inside `main`, correctly bypassing the header, on both the public sign-in page and the authenticated dashboard — but that correctness rested entirely on a browser fallback, not on the page itself. This is exactly the gap WCAG technique G1 exists to close: without real focus on the target, assistive technology gets no confirmation the skip worked.

**Fixed the same day:** added `tabIndex={-1}` (plus `outline-none`, since a programmatically-focused non-interactive container doesn't need a visible ring) to both `<main id="main-content">` elements. Re-verified live on both layouts — `document.activeElement` is now the `<main>` element itself immediately on activation, and the next Tab still lands correctly inside it — and `axe-core` reports zero violations on both a public and an authenticated page after the change.

Also closed, not just carried forward: AC-03's Notes had flagged "a stale-focus artifact after one client-side route transition [that] briefly looked like a skipped header" for this case's scope. Tested directly by triggering a real `+ New Application` Server Action transition and checking `document.activeElement` immediately afterward — it resets cleanly to `<body>`, exactly matching a fresh full-page load, so the next Tab press correctly restarts from the skip link. No focus-order defect on client-side navigation; the earlier AC-03 observation was a testing-timing artifact, not app behaviour.

`accessibility-test-plan.md` → **v1.18**; `TEST-DASHBOARD.md` → **v2.39**; `ADR-TRACEABILITY.md` → **v2.48** (`GAP-58`).

## 2026-08-10 — AC-03 run and marked Pass: the full five-step flow, keyboard-only, via Claude-in-Chrome

Started on the same sandboxed Browser pane used for AC-02, and hit a real methodological problem before recording any result: that pane's synthetic key events cannot genuinely simulate Enter, Space, or Escape. Sending "Space" or "Return" produces a keydown/keyup with `keyCode: 0` and an empty `key` string — not a real key identity — so Chromium's native default-action handling (button activation on Space/Enter, form submission on Enter) never fires. This wasn't inferred from one odd result: the sign-in page's password-visibility toggle appeared to fail on Space and Enter but worked instantly on a real mouse click, and — more decisively — pressing Enter in a text field inside an unrelated `<form>` (ordinary HTML behaviour, nothing to do with this app's code) failed to submit it in the same tool. That's proof the tool was at fault, not the app.

Switched to Claude-in-Chrome — a real Chrome window, confirmed via a genuine `keyCode: 13`/`code: Enter` keydown reaching the page — and the same password toggle worked correctly on the very first real Space press. **Recorded as a reusable environment note in `accessibility-test-plan.md`, not a defect**, since it would otherwise look identical to a real keyboard-accessibility failure to a future session using the same sandboxed tool for this kind of test.

With a genuine keyboard-only surface established, a fresh throwaway Supabase account (admin-API created, deleted after) walked the full flow: sign-in, charity profile setup including a real Charity Commission lookup, a full application via pasted-text guidelines, all 5 Q&A sections written and approved plus one manually-added governance item, and a real `.docx` export. Every expected result held — logical Tab order, Enter/Space activation on every control including a custom `<div role="button">` file-upload dropzone (verified by monkey-patching `HTMLInputElement.prototype.click` to confirm the hidden file input was actually triggered, not just visually inspected), Escape correctly closing the account-menu dropdown with focus returning to its trigger, no keyboard trap anywhere, and all three `GAP-38` tooltip triggers still present and reachable — no regression.

Two of the plan's own step descriptions were corrected to match the built UI rather than left to mislead a future run (the same pattern as AC-01's row 18): the Step 4 "Before you begin writing" gate has no checkboxes, only a single continue button; and Step 5's three confirmation checkboxes are themselves the approval — there is no separate "Approve" button. Citation-link and guideline-viewer reachability couldn't be reproduced this run (needs a PDF-sourced guideline for page markers, and this session's `file_upload` tool rejected a path outside its shared-files allowlist) — the same limitation already recorded in AC-01, not a new gap.

`accessibility-test-plan.md` → **v1.17**; `TEST-DASHBOARD.md` → **v2.38**.
