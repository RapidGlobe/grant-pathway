...
**Open question, deliberately not answered here because it is a dashboard-only fact.** Which Supabase project Vercel _production_ points at. Everything in `IMPLEMENTATION-STATUS.md` implies `grant-pathway-dev`, and it cannot be read from the repo (auth is entirely server-side, so no client bundle exposes it). It needs confirming before the tester starts, because if production points at `grant-pathway-prod` the tester cannot register at all — prod's Resend SMTP was never configured, noted on 2026-07-02 as "should be checked before it does" have a live consumer — and nothing past Step 1 has a schema, since every Phase 6 migration was deliberately scoped to dev only pending P5.4.
