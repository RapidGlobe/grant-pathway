...
- **`GAP-93` — decide and document an output-side truncation policy.** `ADR-AI-007` governs input truncation only; no ADR states what the service promises when an output genuinely can't fit in one call. A product/architecture decision, not an implementation task — still open; `GAP-92` being built doesn't settle this.
