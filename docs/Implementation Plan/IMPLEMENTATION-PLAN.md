...
> **⏳ EXECUTED 2026-08-05 — register written, 15 of 23 items applied, 5 decisions outstanding. `P5.0` is NOT yet complete.** Register: **`docs/Implementation Plan/pre-launch-reconciliation-2026-08-05.md`**. All four scoped work items were run. **This task closes when the five open items are dispositioned, not when the register exists** — leaving them open would reproduce the exact failure the task was created to end.
>
> | Ref             | Open decision                                                                                                                                                                            | Consequence of leaving it                                                                                                                        |
> | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
> | **R-10 + R-11** | Is mobile **supported** (NFR-05's 320px) or **blocked** (`GAP-05`'s below-768px banner)? And what happens in the unspecified **768–1023px** band?                                        | **Blocks `GAP-05`, the next scheduled build in `P5.3`** — and `P5.5`'s "confirm usable on Chrome Android and Safari iOS" step becomes unrunnable |
> | **R-06**        | Does `C1`'s £100 cover total infrastructure **and** API, or AI spend only? Committed `P5.4` spend is already ≈£136 under the strictest reading                                           | `A3`, `A13`, `OBJ-05` and two PRD sections all rest on a figure that means three different things                                                |
> | **R-08**        | Are there **9 or 10** contextual tooltips? `help-and-tooltips-test-plan.md`'s table lists ten while its HT-02 Pass note says "all 9 confirmed"                                           | Either one tooltip was never exercised, or a Pass result miscounts the table directly above it                                                   |
> | **R-09**        | `PDR-UI-008` names `tt-approve-required`, `tt-help-improve` and `tt-prior-export-signoff`, none of which exist in the codebase — renamed during the build, or specified and never built? | Same class as `GAP-21`'s ADR naming a deleted route: a PDR is what a session reads during the mandatory `AGENTS.md` §2 check                     |
>
> **`R-10` is the finding to read first.** The 320px-vs-1024px conflict was identified and formally resolved **once already**, in the retired plan (OI-07: "ADR takes precedence… No action required") — and the resolution never propagated to the live document set. It is the sixth instance of the fix-the-instance-not-the-class pattern this task exists to break, and the only one where the class fix had already been written down.
...
- **Cross-browser** (NFR-05): the full five-step flow in Chrome, Edge, Firefox and Safari on desktop; confirm usable on Chrome Android and Safari iOS
