...
- **Mobile viewport banner (GAP-05):** Add a full-screen banner displayed on viewports below 768px informing users that Grant Pathway is designed for desktop. Banner should block the UI (not just warn) and suggest the user switch to a desktop or laptop browser (ADR-ARCH-005). **Show it below 768px only — the 768–1023px band must stay banner-free** (WJ's decision, 2026-08-05, `P5.0` register refs R-10/R-11): that band is functional-but-unoptimised, not blocked, because iPad portrait and older small laptops are common in small charities. `ADR-ARCH-005` previously set a 1024px minimum while placing the banner at 768px and said nothing about the gap between; its 2026-08-05 amendment now states all three bands, and `NFR-05`, PRD §12.5 and `C16` were corrected in the same pass to stop promising 320px and mobile-browser support.
