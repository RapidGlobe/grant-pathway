...
> **Exit test (added 2026-07-30).** This task is not complete when its checklist is ticked. It is complete when the **`Schema Drift Check` workflow goes green and stays green.** That workflow has been red on every daily run since 2026-07-06 — 23+ days at the time of the Opus audit — because Phase 6's migrations exist only on `grant-pathway-dev`. A permanently-red alarm is not a working alarm, and this is the single largest go-live blocker (audit finding **S1**, dispositioned by WJ 2026-07-29 as planned-and-tracked work belonging here). It also means **every "live-verified" result recorded to date was verified against dev.**
