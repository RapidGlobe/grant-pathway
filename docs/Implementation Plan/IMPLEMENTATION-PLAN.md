...
6. **Text-sizing `px` → `rem` conversion (GAP-79) — decided 2026-08-12, not yet built.** Convert all 384 `text-[Npx]` font-size instances across 30 component/page files to `rem` (Tailwind's own scale classes or arbitrary `rem` values), so text responds to the browser's default font-size preference as `design-requirements` §8.6 requires — found failing, live, during `accessibility-test-plan.md`'s AC-15 Step 4 (doubling the root element's font size directly produced zero change in any sampled `px`-sized text). **WJ's decision:** convert to `rem` rather than amend §8.6 to accept `px` + zoom — some users rely specifically on the font-size preference rather than zoom (zoom forces whole-viewport reflow many of those users are trying to avoid), so the two are not equivalent, and amending the spec would mean formally accepting a real AA gap. **Scope is smaller than the instance count suggests:** the 384 occurrences collapse into 11 distinct pixel values, with `14px`/`13px`/`15px`/`12px` covering roughly 90% — build an 11-row `px`→`rem` lookup table and script the substitution across the 30 files, then re-run AC-15's Step 4 check to confirm it now responds and spot-check a few screens. **Estimated at around half a day** — mechanical, same substitution repeated, no logic changes; at the default font size `14px` and its `rem` equivalent render identically, so no visual regression is expected. **Deferred to a later session** (WJ's choice, 2026-08-12) — not blocked on anything technical. See `GAP-79` in `ADR-TRACEABILITY.md`.
