...
- **Run `@axe-core/react` and fix all violations. This must be run from a local `npm run dev` session — it does not run in a deployed Vercel build,** because the component only mounts when `NODE_ENV === 'development'`. Found live on 2026-07-25, when HT-05's axe-core step was attempted against the deployed site and blocked. The previous wording, "run in development mode", read as advice rather than a constraint.
