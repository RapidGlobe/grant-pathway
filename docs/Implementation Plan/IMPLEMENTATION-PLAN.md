...
- **Sentry route tagging (GAP-21):** Wrap the Bedrock call in `generate-summary` and `refine-answer` routes with `Sentry.withScope(scope => { scope.setTag('route', 'generate-summary') ... })` so errors are filterable by route in the Sentry dashboard (ADR-OPS-005, `technical-design.md §14`).
...
- ~~**Export disclaimer wording (GAP-24):** Fix the disclaimer text in the export route (`app/api/export/[applicationId]/route.ts`). Change from "All content has been checked for accuracy before submission." to "Please review carefully before submitting to the funder." (PDR-DH-003).~~ — **✅ Built 2026-07-30.** Pulled forward out of P5.3 and done immediately: the old wording had the service asserting, in the user's name and on a document sent to a funder, that accuracy had been checked. `PDR-DH-003` amended in the same change to record the built first sentence ("This application was prepared…"), which was a deliberate 2026-06-01 choice matching the Step 5 checkbox.
