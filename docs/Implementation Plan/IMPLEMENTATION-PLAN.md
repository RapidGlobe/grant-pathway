...
- **Migration tracking reconciliation (found and largely resolved 2026-07-01):** `supabase migration list --linked` showed only the first 3 of 17 local migrations as applied on both `grant-pathway-dev` and `grant-pathway-prod` — the CLI's tracking table had been stale since 2026-05-20 because subsequent schema changes were applied by pasting SQL directly into the Supabase dashboard SQL editor rather than via `supabase db push`. A full dev-vs-prod schema diff found production was missing: `reserve_ai_slot`/`update_ai_slot_token_count`/`cancel_ai_slot` (missing on **both** dev and prod — every AI route calls `reserve_ai_slot` first, so AI features were non-functional on any hosted environment), `approve_application`/`reopen_application` (missing on prod only — Step 5 Approve and dashboard Reopen were broken in production), the `charity_paraphrase` enum value (missing on both), and 6 columns across `applications`/`application_answers` (missing on prod only — `assembled_draft`, `draft_status`, `ai_refined_answer`, `is_budget_question`, `char_limit`, `limit_type`). All of this has been applied directly to both dev and prod via the SQL Editor and verified via a full schema re-diff — dev and prod now match exactly.
  ...
  - `grant-pathway-dev`'s tracking table fully repaired via `supabase migration repair --status applied <14 versions> 20260701000000 --linked` — confirmed via `supabase migration list` showing all 18 migrations matched Local/Remote. `supabase db push` is trustworthy against dev again.
  - **Remaining:** run the equivalent `supabase migration repair` against `grant-pathway-prod` from a real terminal (the DB password prompt needs an interactive session, not an AI-issued command) — link with `supabase link --project-ref mvmjryipieepvsjudche`, then repair the same version list once RLS hardening (`20260622000003_rls_hardening.sql`) is confirmed present on prod.
- ~~Apply initial migrations to production Supabase project: `supabase db push --db-url [prod-url]`~~ — superseded by the reconciliation above. Once prod's tracking table is repaired, `db push` will be safe to use for genuinely new future migrations.
