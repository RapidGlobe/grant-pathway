...
- **`GAP-92` — bring `app/api/refine-answer/route.ts`'s truncation handling in line with `GAP-52`'s fix.** `REFINE_MAX_TOKENS` (800) has no `stop_reason` check today — the identical pre-`GAP-52` shape, confirmed by reading the current route directly, and proportionally tighter than `generate-summary` ever was (a baseline 500-word answer is already ~670 tokens before the JSON wrapper or any improvement text). Do not reuse `response_too_long`'s existing message verbatim — it's worded specifically for guidelines/summarising and would read wrong for a refine failure; needs its own `AiErrorCode` or a careful reword. Must be built together with `GAP-94` below, or the new error still won't display correctly.
- **`GAP-93` — decide and document an output-side truncation policy.** `ADR-AI-007` governs input truncation only; no ADR states what the service promises when an output genuinely can't fit in one call. A product/architecture decision, not an implementation task — worth settling before `GAP-52`'s ceiling (or `GAP-92`'s, once built) needs raising again.
- **`GAP-94` (flagged for WJ's decision on whether it belongs in this same pass) — `/api/refine-answer`'s errors show the raw error code to the user, not the friendly message**, because `handleRefine` in `application-step4-draft.tsx` reads `data.error` instead of `data.message`. Affects every error this route returns today (a rate limit currently displays the literal text "rate_limited"), not just truncation — found while investigating `GAP-92`, broader in scope than it.
