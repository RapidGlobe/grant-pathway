# Grant Pathway v1 — Implementation Status (Archive — Phase 0–5)

**Purpose:** Archived entries covering Phase 0 (Project Bootstrap) through Phase 5 (Pre-Launch) — all sub-phases, slices, and gate checks — plus Phase 6 task P6.1 (Profile schema extension).

**Active status:** Current status (Phase 6, P6.2a onward) is in [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md).
...
| **Phase 5 — Pre-Launch**                                                                                     | **9**  | **1**  | In progress — **recounted 2026-07-30; see the note below this table**                                                                                                                                                                            |
| &nbsp;&nbsp;~~**Funder Directory (DR-FD-001)**~~ — **superseded 2026-07-15, no longer counted**              | **0**  | **0**  | ➖ Reversed (`DR-FD-001` v1.4). Previously counted as **6 of 6 ✅ Complete** — the bulk of the old "7 done"                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;~~P5.FD1 — Create `funders` Supabase table and RLS policy~~                          | 0      | 0      | ➖ Superseded — `funders` table now dormant                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;~~P5.FD2 — Seed `funders` table with 12 approved orgs from target funder list~~      | 0      | 0      | ➖ Superseded                                                                                                                                                                                                                                    |
| &nbsp;&nbsp;&nbsp;&nbsp;~~P5.FD3 — Add nullable `funder_id` FK column to `applications` table (migration)~~  | 0      | 0      | ➖ Superseded — `funder_id` dormant, never populated since 2026-07-15                                                                                                                                                                            |
| &nbsp;&nbsp;&nbsp;&nbsp;~~P5.FD4 — Replace free-text funder name input in Step 1 with searchable picker~~    | 0      | 0      | ➖ **Reversed** — Step 1 is free text again                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;~~P5.FD5 — Add "My funder isn't listed — request it" link below picker~~             | 0      | 0      | ➖ Removed with the picker                                                                                                                                                                                                                       |
| &nbsp;&nbsp;&nbsp;&nbsp;~~P5.FD6 — Wire funder request notification to Rapidglobe~~                          | 0      | 0      | ➖ Removed with the picker                                                                                                                                                                                                                       |
| &nbsp;&nbsp;**Performance (ADR-AI-010)**                                                                     | **1**  | **1**  | ✅ Complete — **the only genuinely completed Phase 5 work**                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.PERF1 — Document pre-processing (`lib/preprocess-text.ts`) in `generate-summary`  | 1      | 1      | ✅ Complete                                                                                                                                                                                                                                      |
| &nbsp;&nbsp;**Pre-Launch**                                                                                   | **8**  | **0**  | Not started                                                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.0 — Requirements reconciliation (**new 2026-07-30, blocking; must precede P5.5**) | 1      | 0      | Not started                                                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.1 — Compliance                                                                    | 1      | 0      | Not started                                                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.2 — Security review                                                               | 1      | 0      | Not started                                                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.3 — Accessibility (narrowed 2026-07-30 to genuine accessibility work)             | 1      | 0      | Not started                                                                                                                                                                                                                                      |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.3b — Spec-deviation fixes (**new 2026-07-30**, moved out of P5.3)                 | 1      | 0      | In progress — **1 of 6 built**: GAP-24 ✅ 2026-07-30 (`a3f21f9`). GAP-25, 31, 22, 21 outstanding; GAP-23 deferrable                                                                                                                              |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.4 — Production infrastructure                                                     | 1      | 0      | Not started — **exit test: `Schema Drift Check` green and staying green**                                                                                                                                                                        |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.5 — Final testing (rewritten 2026-07-30 around the `TEST-DASHBOARD.md` layers)    | 1      | 0      | Not started — depends on P5.4; every 🟢 to date was earned against **dev**                                                                                                                                                                       |
| &nbsp;&nbsp;&nbsp;&nbsp;P5.6 — DNS and Go-Live                                                               | 1      | 0      | ⚠️ BLOCKED pending the Phase 6 → Go-Live Gate                                                                                                                                                                                                    |
| &nbsp;&nbsp;&nbsp;&nbsp;_P5.5b — Admin Dashboard (`/admin`)_                                                 | —      | —      | 🕓 **POST-LAUNCH — deliberately excluded from the go-live gate** (WJ, 2026-07-30). Not counted above. Previously in no status table at all, which is finding A3. Day-one statistics come from `supabase/queries/operator-statistics.sql` instead |
| &nbsp;&nbsp;P6.1 — Profile schema extension                                                                  | 1      | 1      | ✅ Complete (2026-07-05) — dev only, see note below                                                                                                                                                                                              |

_(This is the Phase 6 summary row for task P6.1 only, copied here for continuity with Phase 0–5. The full Phase 6 block — P6.2a onward, plus the Phase 6 → Go-Live Gate and Total — is tracked in [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md).)_

> **⚠️ Phase 5 recounted 2026-07-30 — read this before quoting any Phase 5 figure.** This table previously showed Phase 5 as **13 tasks, 7 done**. That was wrong, and wrong in a flattering direction: **six of the seven "done" were the funder-directory tasks (`P5.FD1`–`FD6`), fully reversed on 2026-07-15** by `DR-FD-001` v1.4 — the picker was removed and Step 1 is free text again. `IMPLEMENTATION-PLAN.md` struck them through at the time; this table did not, so it kept counting reversed work as completed work for six weeks. **Phase 5's real progress was one task, `P5.PERF1`.**
>
> The table now reads **9 tasks, 1 done**: the funder-directory block recounted to zero and retained struck through for the audit trail, plus the two tasks added by the 2026-07-30 Phase 5 restructure (**P5.0** requirements reconciliation and **P5.3b** spec-deviation fixes). `P5.5b` (Admin Dashboard) has a row for the first time — it previously appeared in no status table at all, so whether it blocked launch was unanswerable — and is now explicitly **post-launch and outside the go-live gate** (WJ's decision, 2026-07-30), therefore not counted in the nine.
>
> Related and corrected in the same pass: `IMPLEMENTATION-STATUS.md` described this document as "the full Phase 0–5 breakdown (all complete, 97 tasks)". Phase 5 has never been complete. A reader of the live status document reasonably concluded otherwise. Full detail: `docs/Implementation Plan/phase-5-restructure-proposal-2026-07-29.md` §2.1 (findings A1–A4) and `CHANGELOG.md` (2026-07-30).
...
A code review found the Step 3 extraction (`lib/prompts.ts`, `buildSummaryPrompt`) produces a funder-specific `supportingDocuments` list on every call but never renders it — unlike `funderAiPolicy`, which is deliberately withheld with an explanatory comment, this field had none and looked like dead extraction work. Presented WJ two options (display on Step 3, or remove from the prompt); investigation surfaced a third — Step 4's existing "Before you begin writing" preparation checklist (`application-step4-prep-checklist.tsx`, S6.4) shows a hardcoded, identical-for-every-funder list. WJ chose to merge the extracted list into that screen as a second, funder-specific checklist shown alongside the standing one, not replacing it — see `PDR-UI-007`, `AC-FR-28-09`, and the new "Step 4 — Preparation Checklist" entry in `screen-requirements.md` (that screen had no prior entry in that doc at all; the file was later retired and merged into `PRD-Grant-Pathway.md` Section 7 on 2026-07-13 — see the Notes log in [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md) for that entry). `app/(authenticated)/applications/[id]/step/4/page.tsx` now parses `ai_summary` before the `not_started` branch instead of only later. `tsc --noEmit` and `eslint` clean; not yet browser-verified — reaching this screen needs a real AI summarisation call against the monthly quota, left for WJ's own live testing.

---

### 2026-07-10 — Dead code removed — `buildDraftPrompt`

`lib/prompts.ts`'s `buildDraftPrompt` function (and the `ApplicationQuestion` type used only by it) had zero callers since `/api/generate-draft` was deleted 2026-07-01; flagged as a follow-up in `PRD-Grant-Pathway.md` v0.4 rather than fixed there. Removed here along with its dedicated tests in `__tests__/prompts.test.ts`. Re-confirmed zero references via full-codebase grep; `tsc --noEmit`, `eslint`, and the full Vitest suite (22 tests) all clean. `PRD-Grant-Pathway.md` bumped to v0.5.

---

### 2026-07-01 — `Grant-Pathway-Business-Overview.docx` regenerated from `business-overview.md`

The Word export was Version 1.3 (22 May 2026) and still described AI generating first-draft answers from scratch — the model abandoned 2026-05-28 after funder guidance showed AI-generated content disadvantages applications. `business-overview.md` was correctly updated 2026-05-29 and verified accurate against the live Step 4 code; the `.docx` was just never regenerated. Rebuilt via `docx-js` (hand-editing ~1100 lines of Word XML was judged too risky given how much prose changed) reusing the original's cover-page structure and both brand logos; content copied verbatim from the current `.md`, version bumped to 1.4. Verified via `zipfile.testzip()` and a full pandoc round-trip extraction compared against the source line-by-line — no drift.

---

### 2026-07-01 — `/api/generate-draft` and `advanceToStep5` deleted; all doc/diagram references corrected

Both confirmed to have zero callers anywhere in the codebase (verified via `tsc --noEmit`, `eslint`, and the 22-test Vitest suite after deletion — clean). `saveAnswer`'s docstring updated to note `answer_source: 'ai_generated'` is now permanently unused rather than leaving a dangling reference to the deleted route. Swept every current (non-archived) doc for `/api/generate-draft` references: `technical-design.md` → v1.7 (also backfilled 2 missing document-history rows for the v1.5/v1.6 bumps made earlier today), `future-phases.md`, 6 ADR files (illustrative examples corrected to `/api/refine-answer`; formal decisions unchanged), and 2 still-pending P5.3/P5.4 Sentry tasks in this file retargeted so future work doesn't point at a deleted route. `01-system-architecture.svg`/`.png`/`.docx` and `07-application-workflow.svg`/`.png`/`.docx` corrected and regenerated — Step 4's description no longer claims AI generates draft answers.

---

### 2026-07-01 — Dev/prod schema gap closed — AI features and approve/reopen were broken on every hosted environment

Full dev-vs-prod diff found `reserve_ai_slot`/`update_ai_slot_token_count`/`cancel_ai_slot` missing on both projects (every AI route calls `reserve_ai_slot` first — AI features were non-functional wherever the app is actually hosted, only working via local Docker dev), `approve_application`/`reopen_application` missing on prod only (Step 5 Approve, dashboard Reopen broken in production), 6 columns missing on prod only (`assembled_draft`, `draft_status`, `ai_refined_answer`, `is_budget_question`, `char_limit`, `limit_type`), and the `charity_paraphrase` enum value missing on both. Root cause: schema changes since 2026-05-20 were applied via the Supabase dashboard SQL editor rather than `supabase db push`, so the CLI's tracking table only recorded 3 of 17 migrations on both dev and prod. All missing schema applied directly to both projects and verified via a full schema re-diff — dev and prod now match exactly. `item-21-transactional-integrity.sql` relocated from `docs/migrations/` to a proper tracked migration (`supabase/migrations/20260701000000_item21_transactional_approve_reopen.sql`); its original header mislabelled dev's project ref as "prod", almost certainly why the fix reached dev but not production originally. RLS hardening (`20260622000003_rls_hardening.sql`) confirmed genuinely present on prod (`pg_policy` queried directly, not assumed) before repairing it. Both dev and prod migration tracking fully repaired via `supabase migration repair` — WJ ran the prod repair himself from a real terminal, since the DB password prompt can't go through an AI-issued command. `supabase migration list` confirmed all 18 migrations matched Local/Remote on both projects. `supabase db push` is trustworthy against both dev and prod again.

---

### 2026-07-01 — `07-application-workflow.png`/`.docx` regenerated

WJ installed ImageMagick (bundles librsvg) after the tool gap flagged below. Regenerated the PNG at `-density 200` to match the original's exact 2083×3125px output. Also fixed a rendering defect: the `&#9888;` (⚠) warning-sign entity rendered as a missing-glyph box under ImageMagick's local font set (the original browser-based renderer had emoji font coverage); replaced both occurrences with a hand-drawn vector warning-triangle icon so it no longer depends on font/emoji coverage. Updated `.docx` by swapping the embedded image bytes only (same filename/relationship/dimensions, no XML touched) — verified via `zipfile.testzip()` (clean) since LibreOffice isn't installed here for a full visual re-render.

---

### 2026-07-01 — Application-workflow diagram edge cases + route-naming fix

`07-application-workflow.svg` updated with a callout for the Step 2 truncation-warning banner and the `AI_ENABLED` kill-switch "AI unavailable" state (both added to screen-requirements.md on 2026-06-30 but missing from the diagram; that file was later retired and merged into `PRD-Grant-Pathway.md` Section 7 on 2026-07-13 — see the Notes log in [IMPLEMENTATION-STATUS.md](IMPLEMENTATION-STATUS.md) for that entry). Known limitation: no SVG rasteriser or working browser automation was available in this environment (Chrome sandboxing blocked both `file://` and `localhost` navigation) — the `.svg` source is current but `.png`/`.docx` exports are stale pending manual re-export. Separately, fixed a route-naming inconsistency: `technical-design.md` (project tree + 2 route tables, 12 occurrences) and `ADR-ARCH-004-multi-step-flow-state.md` (9 occurrences) referred to `/application/[id]/step/N` (singular); corrected to `/applications/[id]/step/N` (plural) to match the actual codebase and the D1 resolution already recorded in `IMPLEMENTATION-PLAN.md`.

---

### 2026-07-01 — FR-07 (MFA) documentation regression corrected

The 2026-06-30 technical-design.md v1.4 gap-analysis pass incorrectly resurrected a `/mfa` TOTP challenge route (project tree, protected routes list, routes table) that was actually removed from the codebase on 2026-06-12 (`fix: remove orphaned MFA component and page files`) and already correctly demoted to Won't Have in `docs/moscow-feature-register.md`. The regression traced back to a stale `/mfa` entry left in `middleware.ts`'s `PROTECTED` array after the removal. Fixed: `middleware.ts` dead entry removed; technical-design.md v1.5 strips all 3 phantom references; acceptance-criteria.md FR-07 explicitly marked Won't Have (criteria retained for historical reference). Decision confirmed with WJ: MFA stays out of v1, may be reconsidered post-launch if demand warrants it.

---

### 2026-06-29 — Least-privilege key scoping — Alan Knox POST-LAUNCH item 26, VQ-021

AWS: new IAM user `grant-pathway-prod` with custom policy `grant.pathway.bedrock.invoke` (single `bedrock:InvokeModel` permission, scoped to eu-west-2 Claude model); Vercel prod env vars updated. Resend: new send-only API key scoped to `grantpathway.org.uk`; old Full access key deleted; Vercel updated. Charity Commission: read-only by design, no action.

---

### 2026-06-29 — SPF/DKIM/DMARC confirmed — Alan Knox POST-LAUNCH item 27, VQ-022

SPF updated to add `include:amazonses.com` (Resend/SES). DKIM: `resend._domainkey.grantpathway.org.uk` confirmed present and Verified. DMARC: `p=quarantine` confirmed in place. Cross-client rendering deferred to P5.4.

---

### 2026-06-29 — Password policy hardened — Alan Knox POST-LAUNCH item 25, VQ-009

Supabase prod Auth settings updated: min length 6 → 12; password requirements set to letters and digits; leaked password check enabled (HaveIBeenPwned); secure password change enabled; require current password on update enabled.

---

### 2026-06-29 — Branch protection enabled — Alan Knox POST-LAUNCH item 28, VQ-014

GitHub Pro activated. Branch protection applied to `master`: all four CI jobs required (`lint-and-typecheck`, `test`, `audit`, `validate-migrations`); strict mode (branch must be up to date); force-pushes and deletions blocked.

---

### 2026-06-29 — CSP nonce migration — Alan Knox POST-LAUNCH item 22, F-08-02 (M5)

Per-request nonce generated in `middleware.ts`; forwarded as `x-nonce` request header via `lib/supabase/middleware.ts` (preserved across cookie-refresh path); `Content-Security-Policy` with `'nonce-{nonce}'` on `script-src` set on every HTML response, replacing `'unsafe-inline'`. Static CSP removed from `next.config.ts`. Root layout made async (`await headers()`) so Next.js stamps the nonce on its own hydration scripts. TypeScript clean.

---

### 2026-06-29 — Transactional integrity — Alan Knox POST-LAUNCH item 21, F-06-05/06 (M9)

`approve_application` and `reopen_application` Postgres functions created (`docs/migrations/item-21-transactional-integrity.sql`). Each runs both table updates (`applications` + `application_answers`) in one transaction; raises `application_not_found` if ownership check fails. `actions/applications.ts` updated to call `supabase.rpc()` for both actions. `lib/database.types.ts` updated with typed signatures. Scope limited to approve/reopen — `assembleAndAdvance` is serial, deletion paths handled by Supabase Auth cascade. TypeScript clean.

---

### 2026-06-29 — AI kill-switch added — Alan Knox POST-LAUNCH item 20, F-09-03 (M15)

`AI_ENABLED=false` env flag added to all four AI entry points: `generate-summary`, `generate-draft`, `refine-answer` each return HTTP 503 (`overloaded`) immediately before authentication; `actions/charity.ts` skips the Bedrock paraphrase and degrades to empty strings. `AI_ENABLED=true` added to `.env.example` with usage notes. TypeScript clean (0 errors).

---

### 2026-06-29 — SAR procedure documented — Alan Knox POST-LAUNCH item 19, F-05-02 (M12)

`docs/legal/sar-procedure.md` created. Covers: acknowledgement within 3 working days, identity verification, Supabase SQL queries to retrieve all personal data for a given user (auth account, user profile, charity profile, applications, answers, AI usage log), response email template with ZIP attachment, running log of requests, and notes on deadlines and edge cases. Decision: manual process is proportionate at pre-launch scale. Privacy Policy Section 8 already directs users to wjokhia@rapidglobe.com — no policy change required. Automated endpoint deferred to a future phase.

---

### 2026-06-29 — Generated Supabase types applied (Alan Knox POST-LAUNCH item 18, F-06-07 M10)

`lib/database.types.ts` generated from remote project `stanwaejdvlvremtffkf` via `supabase gen types typescript`. Three SECURITY DEFINER RPC function types (`reserve_ai_slot`, `update_ai_slot_token_count`, `cancel_ai_slot`) added manually — CLI missed them. `charity_paraphrase` enum value added manually — was applied via SQL Editor so not captured by gen. `createClient<Database>()` applied to `lib/supabase/client.ts`, `server.ts`, and `middleware.ts`. `AiSummaryData`, `AiSummaryQuestion`, `AiSummarySection` moved from `app/api/generate-summary/route.ts` to `lib/types.ts`; re-exported from route to preserve existing import paths. `actions/applications.ts` update payload re-typed from `Record<string,unknown>` to `{ current_step: number; draft_status?: string }`. TypeScript clean (0 errors).

---

### 2026-06-22 — Charity paraphrase authenticated and metered (independent review item)

`actions/charity.ts` `lookupCharity`: `getUser()` added at top (returns `unavailable` if no session); monthly cap checked fail-closed before Bedrock (error or exceeded → skip paraphrase, degrade gracefully); `aiRatelimit.limit()` applied; `ai_usage_log` inserted on success (`request_type: charity_paraphrase`, `application_id: null`). TypeScript clean (0 errors).

---

### 2026-06-22 — `MONTHLY_CAP` and `MODEL` consolidated into `lib/prompts.ts` (independent review item)

`lib/prompts.ts` now exports `MONTHLY_CAP = 50` and `APPROACHING_LIMIT_THRESHOLD = 40`. All three AI routes import from there; local declarations removed. Duplicate `MODEL` in `actions/charity.ts:16` removed; imports from `@/lib/prompts`. TypeScript clean (0 errors).

---

### 2026-06-22 — AI cap-count made fail-closed — launch-blocker F-01-01 (independent review item)

All three AI routes (`generate-summary`, `generate-draft`, `refine-answer`) updated: `error` now destructured from the `ai_usage_log` count query; returns HTTP 500 (`server_error`) if the query fails instead of defaulting to 0 and bypassing the cap. Prevents unbounded Bedrock cost exposure on DB error. TypeScript clean (0 errors).

---

### 2026-06-22 — Cross-user file-read IDOR closed on upload/process (independent review item)
...
### 2026-05-07 — Plan updated to v1.5 following full review of all 42 ADRs and technical-design.md
...
### 2026-05-20 — Phase 3 complete. P3.10 complete

AWS Budget `grant-pathway-bedrock-cap` created ($127/~£100). Two email alerts: $89 (~~£70 warning) and $127 (~~£100 cap). Scoped to All AWS services for now — narrow to Bedrock once first Bedrock invoice generated. No automated hard-stop action attached (IAM role setup deferred to P5.4 pre-launch).

---

### 2026-05-20 — ADR traceability table created; P3.12 and phase gates added
...
### 2026-05-20 — P3.11 added to Phase 3

`/api/health` endpoint task added following compliance review — ADR-OPS-007 requires the endpoint but no corresponding build task existed in the plan. Phase 3 now has 11 tasks (10 complete). Total plan tasks: 77.

---

### 2026-05-20 — Phase 3 compliance review — 2 High severity fixes applied

(1) CSP `connect-src` in `next.config.ts` updated to include Sentry EU ingest domain (`https://*.ingest.de.sentry.io`) — browser SDK was silently blocked without this. (2) `sentry.edge.config.ts` PII scrubbing (`beforeSend` hook) added — client and server configs already had it; edge was overlooked. Dependencies updated: next 16.2.5 → 16.2.6 (CVE-2026-44575 High severity middleware bypass fixed), @tailwindcss/postcss 4.2.4 → 4.3.0 (PostCSS XSS), @anthropic-ai/sdk 0.97.0 → 0.97.1, tailwind-merge 3.5.0 → 3.6.0. 6 remaining compliance items (Medium/Low) to be addressed before Phase 4.

---

### 2026-05-20 — P3.8 complete
...
### 2026-05-21 — Slice 4 complete

Full file upload pipeline wired: signed URL → XHR direct upload to Supabase Storage → server-side extraction (`lib/extract-text.ts`) → `sessionStorage` storage → `advanceToStep3()` Server Action. Paste path wired. All error states live. Orphan cleanup cron (`vercel.json`, every 30 min) deployed. Session restore and re-upload advisory (GAP-13, GAP-19) implemented. TypeScript clean (0 errors). ⚠️ Confirm cron active in Vercel dashboard after deployment.

---

### 2026-05-21 — Slice 3 complete

Step 1 saves funder/grant names to DB and advances current_step to 2. Reusable `lib/application-guard.ts` helper introduced for step locking across all step pages (2–5). Step 2 now enforces step locking via `getApplicationOrRedirect`. TypeScript clean (0 errors). No manual steps required.

---

### 2026-05-21 — Slice 2 complete

Dashboard fetches real applications and AI usage count. `createApplication()` creates a DB row and redirects to Step 1. Continue navigates to stored `current_step`. View (approved/exported) triggers re-open flow. Delete with status-specific confirmation removes the row and refreshes the list. TypeScript clean (0 errors). No manual steps required — all Slice 2 tables and GRANTs were already in place from Phase 3.

---

### 2026-05-21 — Logo placeholder recorded

`components/logo.tsx` marked with `⚠️ PLACEHOLDER LOGO — replace before launch` comment. Real Grant Pathway logo (SVG/PNG, light + dark variants) to be supplied by Wac / RapidGlobe Ltd before P5.4 production launch. Instructions for swapping in the real asset are embedded in the component.

---

### 2026-05-21 — S1.4 complete. Slice 1 complete

Dashboard page fetches real first name and profile existence; banner shown/hidden correctly on both empty and populated states. `MOCK_PROFILE_INCOMPLETE` removed from `dashboard-populated.tsx`.

---

### 2026-05-21 — S1.3 complete

`getCharityProfile()` fetches existing profile from `charity_profiles`; `CharityProfileData` type exported; form `initialData` prop pre-fills all five fields; `isEdit` now derived from DB state not URL param. TypeScript clean.

---

### 2026-05-29 — GAP-27 and GAP-28 raised during pre-test review of Idlewild Trust question sets

⚠️ **GAP-27 — Character limits not supported (Medium).** Idlewild Arts and Conservation forms use character limits (800 chars, 1600 chars), not word limits. `word_limit` in `application_answers` and the Step 4 word counter only handle word counts. The AI prompt extracts `wordLimit` only — character limits will be missed or incorrectly converted. Fix: add `limitType: 'words' \| 'characters'` to `AiSummaryData.questions` and `AiSummarySection`; update AI prompt and Step 4 display.

⚠️ **GAP-28 — Non-text questions extracted as text (Medium).** Idlewild forms include Yes/No, dropdown, date, number, table, and file upload question types. The AI cannot distinguish type from a PDF reference and will extract all as text fields, showing them as textareas in Step 4. Fix: extend AI prompt to extract `questionType`; filter or flag non-text questions in Step 4.

Both gaps affect all portal-based structured funders. Do not use Idlewild as a production test fixture until resolved. Three Idlewild PDFs added to `docs/test-fixtures/`.

---

### 2026-05-22 — Phase 4→5 gate check complete

Full sweep of all 26 ADR gaps against Phase 4 implementation. GAP-07 (null `answer_text` in export), GAP-13 (cron rate-limiter exclusion), GAP-19 (guidelines re-upload advisory) confirmed resolved. Six new gaps identified: GAP-21 (Sentry route tagging in AI routes), GAP-22 (timeout inactivity message), GAP-23 (`loading.tsx` Suspense boundaries), GAP-24 (export disclaimer wording vs PDR-DH-003), GAP-25 (Zod absent from `actions/applications.ts` + `actions/auth.ts`), GAP-26 (`applications/[id]/page.tsx` stub — High severity, must fix before P5.5). All findings recorded in ADR-TRACEABILITY.md. **Gate signed off by WJ, 2026-06-17** — see ADR-TRACEABILITY.md v2.2 for full sign-off narrative.

---

### 2026-05-22 — Slice 8 complete. Phase 4 complete

S8.1: `changePassword()` SA verifies current password via `signInWithPassword` before calling `updateUser`; `wrong_password` state maps to inline field error. S8.2: `POST /api/account/delete` cascade-deletes all user data (application_answers → applications → charity_profiles → ai_usage_log → user_profiles → auth.admin.deleteUser); sends Email 2 (confirmation) via Resend; sign-in page shows green banner on `?deleted=true`. S8.3: `lib/emails/` module created with four email builders; two inactivity cron routes (`/api/cron/inactivity-warning` daily 08:00 UTC, `/api/cron/inactivity-deletion` daily 09:00 UTC) page through `auth.admin.listUsers()` and handle the 23/24-month windows; `vercel.json` updated. Email helper uses Resend REST API via `fetch` (no new dependency). TypeScript clean (0 errors). ⚠️ Confirm both new cron jobs active in Vercel dashboard after next deployment. ⚠️ Set `RESEND_API_KEY` in Vercel env vars for emails to send.

---

### 2026-05-21 — S1.2 complete

`saveCharityProfile()` Server Action added to `actions/charity.ts`; Zod validation, Supabase upsert on `user_id` conflict, `lookup_source` correctly set to `charity_commission` or `manual` (D14). Form wired with real save, loading state, and error banner. TypeScript clean (0 errors).

---

### 2026-05-08 — Phase 0 implementation started

Next.js 16.2.5 scaffold created (Turbopack, React 19, Tailwind v4). Key deviations from plan: (1) Tailwind v4 has no `tailwind.config.ts` — design tokens added via `@theme inline` in `globals.css` instead. (2) Next.js 16 deprecates `middleware.ts` in favour of `proxy.ts` with `export function proxy()` — plan's middleware stub updated accordingly. (3) shadcn `toast` component deprecated — replaced with `sonner`. (4) shadcn `form` component not available in shadcn 4.7.0 registry — to be created manually in Phase 1 using react-hook-form directly. P0.2–P0.5 complete; P0.6 pending GitHub push and Vercel link (manual steps for owner).
...
- [x] **P3.7** Sentry (EU region) configured; `beforeSend` PII scrubbing in place; new-error-type alerts configured
  ...
  - SENTRY_DSN and NEXT_PUBLIC_SENTRY_DSN set in `.env.local`. ~~**production Vercel DSN is empty — to be set at P5.4**~~ — **CORRECTED 2026-07-30. This line was stale and it did real damage.** The production DSN _was_ set at some point after this P3.7-era note was written, and the note was never updated. On 2026-07-29 the Opus audit read this line, took it as current, and published a Severe finding stating that production had no error visibility during an external tester beta. **That finding was false and has been withdrawn.** Verified directly: `vercel env ls` shows both `SENTRY_DSN` and `NEXT_PUBLIC_SENTRY_DSN` present in the Production environment, the deployed production bundle carries a live DSN on `o4511417358745600.ingest.de.sentry.io` (the correct EU region, matching the org ID in `IMPLEMENTATION-PLAN.md`), and WJ confirmed from the Sentry dashboard that `production` events are arriving. **Production error reporting works and has done for some time.** Genuinely still outstanding from the same P5.4 item, and not to be confused with the above: no Sentry **alert rule** for new issues appears to exist (seven issues accumulated unnoticed), PII scrubbing is unconfirmed, and `SENTRY_AUTH_TOKEN` is absent so there are no releases and no source maps (audit O11/O12) — all now explicit line items in P5.4.
...
- [ ] **P5.4** Production infrastructure: Vercel Pro active; `maxDuration = 90` on AI routes confirmed; all production env vars set (including `AI_ENABLED=true`); migrations applied to production Supabase; Sentry production configured; Resend domain verified; email templates confirmed
  ...
  - ⚠️ **Tidy-up: Supabase CLI is out of date (noted 2026-07-02).** Local install is v2.100.1; v2.109.0 is available. Low priority — not blocking any current work — but worth updating before it drifts further, since a large enough version gap can occasionally change CLI output formats or flags relied on by RT-00 and the schema-drift-check workflow's local reproduction steps. Run the installer refresh from https://supabase.com/docs/guides/cli/getting-started#updating-the-supabase-cli whenever convenient.
- [ ] **P5.5** Final testing: full five-step flow on production; all error states tested (including large-document warning); account deletion tested; both export formats tested (Word export opened in Word to verify structure, font, disclaimer); returning user flow; session timeout confirmed; cross-browser test (Chrome, Edge, Firefox, Safari desktop; Chrome Android, Safari iOS); AI performance confirmed (summary ≤30s, draft ≤60s); **feedback opt-in verified** (register with box ticked and unticked; confirm `feedback_consent` value written correctly to `user_profiles` in Supabase). ⚠️ Post-launch: query `user_profiles` where `feedback_consent = true` and establish a process to contact those users — do not let the data go unused.
  - ✅ **D-012 found and fully resolved (2026-07-02):** the first live run of RT-01a (account registration, dev) found two chained defects — a stale Resend SMTP API key blocking all verification email delivery, then (once fixed) Gmail's own server-side link scanning silently consuming every verification link within seconds of sending, regardless of browser. Fixed by no longer completing verification on page load; `/auth/callback` now redirects signup confirmations to a new `/verify-email/confirm` page that confirms automatically via JavaScript with no visible action, closing the scanner gap while staying single-click for a real user. Re-verified clean across Comet, Chrome, and Edge. Full detail in `CHANGELOG.md` and `regression-test-plan.md`'s Defect Log.
