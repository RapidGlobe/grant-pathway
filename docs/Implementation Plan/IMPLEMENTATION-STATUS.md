...
**Last updated:** 2026-06-21 (Security fixes: middleware.ts wired up; IDOR/BOLA fix; 6 Dependabot vulnerabilities resolved; npm audit clean)
**Plan version:** 2.8
...
- [ ] **P5.4** Production infrastructure: Vercel Pro active; `maxDuration = 90` on AI routes confirmed; all production env vars set; migrations applied to production Supabase; Sentry production configured; Resend domain verified; email templates confirmed
  ...
  - ⚠️ **Production Supabase project may be paused — unpause as the first P5.4 step (decided 2026-06-10):** `grant-pathway-prod` (ID `mvmjryipieepvsjudche`) is on the free tier with no API activity (production env vars still point at the dev project), so Supabase auto-pauses it after 7 days of inactivity. Wac decided to accept the pause warnings rather than upgrade early — the project holds no schema or data, restore is one click within 90 days, and Phase 5 is expected within 2–3 weeks (well inside the window). When P5.4 begins: (1) unpause `grant-pathway-prod` from the Supabase dashboard; (2) activate Supabase Pro on it per ADR-DATA-005 (ends pausing permanently and enables daily backups); (3) apply migrations; (4) switch production env vars from the dev project. Do NOT switch env vars before P5.5 final testing is ready — funder testing currently runs against the production URL using the dev database.
