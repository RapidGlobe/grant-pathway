...
| 9.11 Guideline Source-Reference (Citations)                  | FR-48                  | ✅ Reviewed 2026-07-13, updated 2026-07-14 — `P6.2a` groundwork now built (extraction tags structure, invisible to users); citation display itself remains a `P6.3`–`P6.5` target, not yet started                                                                                   |
...
_Added 2026-07-10. FR-48 was introduced the same day this section was added — `PDR-DH-004` and `ADR-DATA-007` formalise the "Option 2" design and architecture; `ADR-DATA-006` hosts the citation field in the item-graph schema. **Updated 2026-07-14:** `P6.2a` (extraction/pre-processing groundwork) has landed — see below — but this is invisible plumbing only; no citation is shown to a user anywhere yet. `P6.3`–`P6.5` have not started. Every criterion below still describes target behaviour once built, not current behaviour — none of it should be treated as currently passing, and test plans should not report against it as a defect until Phase 6 lands._
...
**Not built — as of 2026-07-14, the citation itself does not exist yet, though its extraction groundwork now does.** `P6.2a` (2026-07-14) changed `lib/extract-text.ts` to extract PDFs per-page with `[PAGE N]` markers (was `mergePages: true`, flattening pages away) and docx with `[SECTION: A > B]` markers from Word headings (was headingless `extractRawText`); `lib/preprocess-text.ts` no longer strips these markers and its truncation safety net is now marker-aware. **None of this is surfaced to a user or recorded anywhere yet** — `application_items`'s `guideline_reference` column (`P6.2`, built) is not populated by any extraction code path, and no "view original guidelines" panel or PDF-viewer component exists in `components/` or `app/`. This FR depends on `P6.3`–`P6.5` (per `IMPLEMENTATION-PLAN.md`), not yet started, and is part of the Phase 6 → Go-Live Gate.
