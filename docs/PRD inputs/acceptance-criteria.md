...
| 9.11 Guideline Source-Reference (Citations)                  | FR-48                  | ✅ Reviewed 2026-07-13, updated 2026-07-14 — `P6.2a`/`P6.3` (first milestone) now built (structure tagged, citations recorded in the database); still invisible to users, display itself remains a `P6.4`–`P6.5` target                                                              |
...
_Added 2026-07-10. FR-48 was introduced the same day this section was added — `PDR-DH-004` and `ADR-DATA-007` formalise the "Option 2" design and architecture; `ADR-DATA-006` hosts the citation field in the item-graph schema. **Updated 2026-07-14:** `P6.2a` (extraction/pre-processing groundwork) and `P6.3` (first milestone — citations recorded in the database) have both landed — see below — but this is still invisible plumbing only; no citation is shown to a user anywhere yet. `P6.4`–`P6.5` have not started. Every criterion below still describes target behaviour once built, not current behaviour — none of it should be treated as currently passing, and test plans should not report against it as a defect until Phase 6 lands._
...
**Not built — as of 2026-07-14, citations are now recorded but not displayed anywhere.** `P6.2a` (2026-07-14) changed `lib/extract-text.ts` to extract PDFs per-page with `[PAGE N]` markers (was `mergePages: true`, flattening pages away) and docx with `[SECTION: A > B]` markers from Word headings (was headingless `extractRawText`); `lib/preprocess-text.ts` no longer strips these markers and its truncation safety net is now marker-aware. `P6.3` (2026-07-14, first milestone) then changed `lib/prompts.ts` to ask the AI for a citation per question/section, validated against those real markers by new `lib/guideline-citations.ts` before being written into `application_items.guideline_reference` (`P6.2`'s column, populated for the first time). **Still not surfaced to a user anywhere** — no "view original guidelines" panel or PDF-viewer component exists in `components/` or `app/`, and the Step 3 summary bullets (`aboutGrant`/`whoCanApply`/`lookingFor`/`keyRequirements`) are not citation-tagged at all (only questions/sections are, per `P6.3`'s confirmed first-milestone scope). **GAP-33 fixed the same day:** the guideline text a citation would need to display against is now retained (new `application_guidelines` table) — previously neither the raw file nor the extracted text survived past Step 3, which would have left `P6.4`'s viewer with nothing to render. This FR depends on `P6.4`–`P6.5` (per `IMPLEMENTATION-PLAN.md`), not yet started, and is part of the Phase 6 → Go-Live Gate.
