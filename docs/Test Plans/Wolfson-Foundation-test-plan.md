...
|----|------|-------------|----------|--------|
| D-WF-02 | IT-WF-10 | Plain text download shows the same success message as the Word download but no `.txt` file is saved. After a successful docx download (`isExported = true`), clicking "Download as plain text" correctly triggers the re-export warning dialog; on confirmation, `doDownload('txt')` is called, the API returns 200 with `text/plain` content, but the browser silently suppresses the programmatic `a.click()` download — likely because the click is now one async level removed from the original user gesture (inside a dialog callback), which Chrome/Edge treat as a pop-up. The success state updates (`setApprovalStatus`, `setLastExported`) fire regardless, making it appear the download succeeded. **Workaround:** none identified — the txt file is not downloaded. **Fix required:** replace programmatic `a.click()` with a direct `window.location.href` or `window.open` call for the txt format, or use a `<a href>` link element rendered in the DOM rather than a dynamically created one. | Medium | Open |
