...
**Fixture overwritten, 2026-07-27 (same day):** while starting the AB Charitable Trust flagship's v2.1 re-run, this account's charity profile was reset to Asylum Justice's details before the mix-up was caught (the flagship then correctly moved to a new account, `+ABC2@gmail.com`, for the rest of that run). This account no longer holds the Harry's Rainbow profile the above result depended on — the recorded EL-02 pass still stands as historical evidence, but this account can't be used to reproduce or re-verify it. If this case needs re-running from scratch, either restore a Harry's Rainbow-equivalent profile on this account or seed a fresh one.
