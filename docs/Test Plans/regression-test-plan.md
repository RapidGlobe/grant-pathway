...
**Version:** 2.22
**Date:** 2026-06-15
**Last updated:** 2026-08-19 (**RT-02 also passes on production; RT-03 BLOCKED until an application exists there — testing moves to a flagship end-to-end plan, which creates that state.** Also: **`P5.5` restarted and RT-00, RT-01a and RT-01b all pass against production — the first clean runs of the smoke tests anywhere but dev.** Also: **RT-00 strengthened — it now runs the parity audit and one real AI call, because the version that passed on 2026-08-18 did so over both `D-020` and `D-018`. Its Pass should not be carried forward.** Also: **D-020 has a second instance** — found by the environment parity audit, repaired, re-run clean. **D-017 CLOSED as not a defect** — Sentry was working all along and the "no events in 14 days" reading was wrong. See Document History v2.18 and v2.19)
