...
**Version:** 2.14
**Date:** 2026-06-15
**Last updated:** 2026-08-06 (RT-14 gains an optional Storage-deletion add-on for `GAP-47` — see Document History v2.14)
**Status:** RT-00 through RT-15 all Pass (RT-03, RT-09 and RT-11 with a caveat). ⚠️ **RT-09's caveat is the one that matters:** it passed on 2026-07-28 over a live defect — the Word export discards every line break in an answer (`GAP-41`, Defect Log D-015), found 2026-08-06 in a real user's downloaded document, not by this plan. **`GAP-41` was fixed later the same day**, with automated coverage that generates a real `.docx` and counts `<w:br/>` — but RT-09 itself still needs a manual re-run with a genuinely multi-line answer, because no automated test opens the finished file in Word. RT-03 and RT-11's caveats are stale plan text, corrected; RT-14 confirms the 2026-07-23 `service_role` grant fix holds on `grant-pathway-dev`, `grant-pathway-prod` still unpatched; RT-15 closed after fixing two real defects found via diagnostic re-testing — see Defect Log D-013/D-014.
**Tester:** WJ
...
| RT-09   | Final review, approval, and Word export               | Full  | Pass (caveat) | 2026-07-28 | ⚠️ Passed over a live defect — `GAP-41`/D-015. **Fixed in code 2026-08-06 and covered by a packed-`.docx` test; manual re-run with a multi-line answer still required** before this returns to a clean Pass. |
...
**Notes:** Live-tested 2026-07-28, no issues — **but that pass does not cover the new line-break fidelity step, and must not be read as if it does.** `GAP-41` was live in the code on 2026-07-28 and this test passed straight over it, because the answers used were single paragraphs. The result stands for what it checked. **`GAP-41` was fixed on 2026-08-06** — automated coverage now generates a real `.docx`, unzips it and counts `<w:br/>`, and the old code was confirmed to produce zero — but **this case still needs re-running by hand with a genuinely multi-line answer**, because no automated test opens the finished file in Word. Until then RT-09 stays Pass (caveat). See Defect Log D-015.
