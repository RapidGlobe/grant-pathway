...
**Version:** 2.19
**Date:** 2026-06-15
**Last updated:** 2026-08-19 (**D-020 has a second instance** — found by the environment parity audit, repaired, re-run clean. **D-017 CLOSED as not a defect** — Sentry was working all along and the "no events in 14 days" reading was wrong. See Document History v2.18 and v2.19)
...
4. In a terminal, change into the repository root — the directory containing `supabase/` and `package.json` (path corrected 2026-07-30: this step previously hardcoded the author's full local OneDrive path, which `AGENTS.md` §4 forbids in documentation because it is machine-specific and will differ for any other contributor). The CLI needs to be run from inside the project directory to find the linked project config), then run `supabase migration list --linked` (after linking the CLI to the correct project) and confirm every row shows a matching Local/Remote timestamp — no blanks.
