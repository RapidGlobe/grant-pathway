...
**Version:** 2.8
**Date:** 2026-06-15
**Last updated:** 2026-07-28 (RT-15 fresh re-run failed to reproduce — diagnostic with shortened timer underway)
**Status:** RT-00 through RT-14 all Pass (RT-03 and RT-11 with a caveat — plan text was stale, corrected; RT-14 confirms the 2026-07-23 `service_role` grant fix holds on `grant-pathway-dev`, `grant-pathway-prod` still unpatched). RT-15's 2026-07-28 fresh scripted re-run (Chrome) showed neither the warning modal nor the sign-out — under investigation, see RT-15 notes.
**Tester:** WJ
...
| RT-15   | Session timeout (inactivity)                          | Full  | Investigating | 2026-07-28 | Fresh scripted re-run (Chrome) showed no warning modal and no sign-out. Diagnostic underway — see RT-15 notes. 2026-07-04 Pass was a real occurrence, not a scripted run — see below. |
...
**Result:** ☐ Pass &nbsp;&nbsp; ☐ Fail &nbsp;&nbsp; ☑ Investigating &nbsp;&nbsp; ☐ Blocked
...
**Notes (2026-07-28):** WJ attempted a genuine scripted re-run of this test case in Google Chrome and saw neither the 55-minute warning modal nor the 60-minute sign-out — the session simply stayed active. Leading theory: Chrome throttles or suspends `setTimeout`/`setInterval` timers in backgrounded/inactive tabs (and Chrome's Memory Saver can discard a tab's JS execution context entirely), which would explain a real occurrence surfacing once (2026-07-04, where the tab may have stayed foregrounded or the OS never fully suspended it) but not reproducing reliably on a deliberate background wait — this has not yet been confirmed as the root cause. To isolate whether the sign-out _logic_ itself is sound (as opposed to being masked by Chrome's tab-timer behaviour over a full 55/60-minute wait), `WARNING_MS`/`TIMEOUT_MS` in `components/session-timeout-provider.tsx` have been temporarily shortened to 1/2 minutes so the behaviour can be observed directly within a few minutes. **This is a diagnostic-only change, not a product change** — the values will be reverted to 55/60 minutes once this is confirmed one way or the other. Re-test: sign in, leave the tab idle (foregrounded, not switched away from) for 1 minute, confirm the warning modal appears, then wait a further minute without dismissing it and confirm sign-out occurs. If it fails to fire even at this shortened interval, the defect is in the timer/effect logic itself, not Chrome's background throttling — worth also trying with the tab left in the foreground throughout, since that isolates the Chrome-throttling theory specifically.
