...
| IT-HSF-04 | Eligibility check — observe outcome; IT-11 escape hatch if mismatch             | Yes          | N/A             | ✅ Pass | Branch A — Harry's Rainbow passed eligibility (bereaved children, MK deprived areas). IT-11 escape hatch first verified 2026-06-05 (Branch B, ADR-AI-010 retest). D-HSF-03 fix confirmed 2026-06-07: escape hatch flow (profile correction → new application → regenerated summary) now populates Step 4 correctly on first load — 8 questions shown without workaround. |
...
| IT-HSF-09 | Answer approval and Step 5 navigation                                           | No           | N/A             | ✅ Pass | Assembly and approval flow correct. Retest 2026-06-07 (escape hatch flow): approval and Step 5 navigation confirmed working end-to-end.                                                                                                                                                                                                                                  |
| IT-HSF-10 | Word document export — structure and content                                    | No           | N/A             | ✅ Pass | Export correct. D-HSF-01: funder name shows "Henry Smith Charity" not "Henry Smith Foundation" — seed data defect, fixed during this session. Retest 2026-06-07 (escape hatch flow): both Word and text file exports produced correctly. Full end-to-end flow verified.                                                                                                  |
...
| D-HSF-03 | IT-HSF-04 | Step 4 shows "No specific questions were found" fallback on first load after IT-11 escape hatch flow (profile correction → new application → regenerated summary). Sections/questions visible on second load (go Back → regenerate summary). Root cause: Next.js Router Cache timing issue — `setDraftInProgress` Server Action called `redirect()` inside `startTransition`, which caused a soft navigation that served a stale cached render of Step 4 before the question sync ran. A first fix (55c60a7, 2026-06-07) hardened the sync in the Server Component (explicit upsert error checking, null filtering, ignoreDuplicates: false) but did not resolve the issue. Second fix: (1) question sync moved into `setDraftInProgress` so rows exist in the DB before Step 4 renders; (2) `window.location.href` hard navigation used after `{ ok: true }` return, bypassing the Router Cache entirely. Same pattern as D-001 sign-out fix. Retested with Henry Smith Retest application — 8 questions appeared on first load (2026-06-07). | Medium   | Fixed  |
...
| 1.3     | 2026-06-07 | Rapidglobe Ltd | D-HSF-03 fix attempt 1 (55c60a7): hardened upsert in Server Component. Retest failed — root cause identified as Next.js Router Cache timing, not upsert logic.                                                                                                          |
| 1.4     | 2026-06-07 | Rapidglobe Ltd | D-HSF-03 fix attempt 2: sync moved to setDraftInProgress action; window.location.href hard navigation added. D-HSF-03 status updated to "Fix attempted — retest required".                                                                                              |
| 1.5     | 2026-06-07 | Rapidglobe Ltd | D-HSF-03 retest passed — 8 questions appeared on first load of Step 4 (Henry Smith Retest application). D-HSF-03 marked Fixed.                                                                                                                                          |
| 1.6     | 2026-06-07 | Rapidglobe Ltd | IT-HSF-04 escape hatch scenario retested end-to-end: profile correction → new application → regenerated summary → prep checklist → Step 4 first load. 8 questions displayed immediately. IT-11 escape hatch fully verified and closed.                                  |
| 1.7     | 2026-06-07 | Rapidglobe Ltd | Full end-to-end retest completed via escape hatch flow — Q&A, approval, Step 5, Word and text file exports all confirmed working. IT-HSF-09 and IT-HSF-10 notes updated.                                                                                                |
