...
MK Community Foundation funds charities and voluntary organisations delivering activities and services that benefit residents of **Milton Keynes**. Oak Grants support projects costing between £5,001 and £15,000, with a **20% match funding requirement**. Guidelines and portal questions are pasted or uploaded per the standard flow, from `docs/Grant Org Guidelines/MK Comm oak-grants-criteria-final-nov-2025.pdf` (filename corrected 2026-07-30, audit finding **L6** — see "Guidelines — access before testing" below, and do not substitute a freshly downloaded copy).
...
**Use the file already in the repository: `docs/Grant Org Guidelines/MK Comm oak-grants-criteria-final-nov-2025.pdf`.**

⚠️ **Do not download a fresh copy.** This section previously said to obtain the criteria and save them to a `mkcf-oak-grants-criteria.pdf` path "if not already present" — a filename that has never existed (audit finding **L6**, corrected 2026-07-30). Following it literally would not have blocked a tester; it would have led them to conclude the file was absent, download the current version from MKCF, and save a **second copy under a different name**. That matters more than a duplicate file: the repository copy is explicitly the **November 2025** edition, and a copy downloaded now may not be the same document. Every recorded MKCF result — including the extracted question count, which has moved across runs — is measured against the November 2025 criteria. Testing against a silently different edition would invalidate that comparison without anyone noticing.

If the guidelines genuinely need refreshing to a newer edition, treat that as a deliberate change: add the new file alongside the old one under a dated name, update this plan and its expected results, and record it in the Document History. Do not overwrite or substitute.
