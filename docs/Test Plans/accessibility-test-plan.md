...
**Version:** 1.9
**Date:** 2026-08-09
**Status:** In execution. This is P5.3's output artefact and its definition of done. **AC-01 is now fully swept across two runs (rows 1–20 on 2026-08-05, the remaining rows on 2026-08-09) and has FAILED with five defects, including a WCAG Level A keyboard failure (`GAP-49`) and a WCAG AA contrast failure only visible once an answer is actually approved (`DEF-05`).** AC-02 onward have not started. **Read AC-01 from the top — it assumes no prior experience of browser developer tools.**
**Tester:** WJ
...
| ------- | ---------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-01   | axe-core clean sweep, all routes               | ❌ Fail | **Fully swept across two runs** (rows 1–20 on 2026-08-05, remaining rows on 2026-08-09). 5 defects: `DEF-01` contrast (fixed), `DEF-02` target-size (fixed), **`DEF-03` `scrollable-region-focusable` — WCAG 2.1.1 Level A, `GAP-49`** (open), `DEF-04` broken axe auto-reporting (open, feeds `GAP-15`), **`DEF-05` contrast on the "Answer approved" text, only visible once an answer is approved** (open) |
...
**Result:** ☐ Pass &nbsp;&nbsp; ☑ **Fail** &nbsp;&nbsp; ☐ Blocked — _fully swept across two runs: rows 1–20 on 2026-08-05, the remainder on 2026-08-09. Coverage is now complete._
...
**Two of five defects are fixed; three are open.** `DEF-01` and `DEF-02` — fixed 2026-08-07, WJ's decision. `DEF-03` is tracked as `GAP-49`; `DEF-04` feeds `GAP-15`; `DEF-05` is newly logged and unfixed, needing the same kind of design call `DEF-01` needed (no code change applied in this pass — recording only, matching how `DEF-03` was handled on discovery).
