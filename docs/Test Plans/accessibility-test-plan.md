...
**Version:** 1.17
**Date:** 2026-08-10
**Status:** In execution. This is P5.3's output artefact and its definition of done. **AC-01, AC-02 and AC-03 all PASS.** AC-01: all seven logged defects fixed. AC-02: every one of 9 pages scored 100/100 on Lighthouse's Accessibility category, against a 95+ bar; one recurring finding (`GAP-57`, a decorative avatar-letter mismatch against an experimental, zero-weight axe rule) accepted as a tooling limitation rather than fixed. AC-03: the full five-step flow, keyboard-only, via Claude-in-Chrome — a real browser, needed because the sandboxed Browser pane used for AC-02 cannot genuinely simulate Enter/Space/Escape (see AC-03's Notes for how that was caught and confirmed, not assumed). AC-04 onward have not started. **Read AC-01 from the top — it assumes no prior experience of browser developer tools.**
**Tester:** WJ
