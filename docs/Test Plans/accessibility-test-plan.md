...
| 11b   | `/profile`, charity lookup attempted                   | 2              | `color-contrast` ×2                                 | Lookup **unreachable** — `CHARITY_COMMISSION_API_KEY` was a stub locally. One reading discarded as an artefact, see below |
| 11c   | `/profile`, result picked and populated                | —              | —                                                   | **Blocked** — the lookup cannot return results to pick from                                                               |
...
**Still to sweep in run 2:** rows 10 (note only), 11b/11c (needs the Charity Commission key), 18's citation pop-up, 19, 20.2, 20.3, 20.5, 21, 22, 23/24, plus the two manual checks above.
...
**The coverage is partial and the case stays open.** Rows 10, 11b, 11c, 18's citation pop-up, 19, 20.2, 20.3, 20.5, 21, 22, 23 and 24 are unswept, and two `incomplete` findings need a human verdict. Do not read this Fail as the final answer — run 2 can only add findings, not remove them.
...
| -------- | ----- | -------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `DEF-01` | AC-01 | serious  | 1.4.3 Contrast (Minimum), AA      | **One colour token, not many bugs.** `#94A3B8` fails AA on every background it is used on. Five variants, listed in the table below. Overlaps AC-10 — recorded here, not twice.                                                                                                    | Open — needs a design decision, see below                 |
| `DEF-02` | AC-01 | serious  | 2.5.8 Target Size (Minimum), AA   | **One component, six instances.** The show/hide-password eye button renders 16×16 against a 24×24 minimum: sign-in (1), `/register` (2), `/account` (3). Overlaps AC-11 — recorded here, not twice.                                                                                | Open                                                      |
...
All five are below the 4.5:1 floor AA requires for text under 18.66px. **This is a token decision rather than a set of page bugs** — changing what `#94A3B8` resolves to fixes the first three rows everywhere at once, and the two pills need their own pairing. It is a design change and belongs to WJ, not to a code fix chosen here.
