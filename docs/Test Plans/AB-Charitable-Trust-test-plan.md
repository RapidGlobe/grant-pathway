...
**Version:** 2.4
**Date:** 2026-07-27
**Status:** ⚠️ **Production run in progress, 2026-08-19 (`P5.5`)** — results recorded per case as `Notes — 2026-08-19, production run`. **The 2026-07-27 results below were earned on `grant-pathway-dev` and are not evidence about production**; they stand as their own record. **COMPLETE on production, 2026-08-19 — ABC-01 through ABC-10 all Pass, one caveat.** AI summary generated in 22 seconds; the Step 3 banner reported 3 questions plus 1 financial detail, matching the dev run exactly; B4's 15-word limit correct; the dev run's governance-dropdown defect did not recur. ⚠️ **ABC-10 is Pass (caveat): the export's multi-line fidelity check was not exercised**, because the answers written were single-paragraph prose — the exact hole through which `RT-09` passed over `D-015`. **One multi-line export against production is owed**, to be covered in the MK Community Foundation run. ⚠️ **ABC-06's "D5 must not appear" expectation is under active review** — see `docs/impact-assessment-supporting-document-questions.md`. ⚠️ **Account substitution for the whole production run: `grantpathway+RT01test@gmail.com`**, signed in rather than registered, to limit the number of real accounts created on production (WJ). Permitted by the flagship coverage rule's "registration **or login** for returning test user"; registration is covered by `RT-01a` on production the same day. **Previously: fully executed 2026-07-27 under Asylum Justice — ABC-01 through ABC-10 all Pass. One real defect found and fixed same session (ABC-08, manually-added governance dropdown stuck at its default) — see Defect Log.
**Tester:** WJ
...
**Notes — 2026-08-19, production run: ⚠️ Pass (caveat).** The export completed, the `.docx` opened in Word, and nothing was corrupted or missing. ⚠️ **But the multi-line fidelity check was not exercised, and that is the whole reason the check exists.** The answers written for this run were single-paragraph prose, so **there were no line breaks, blank lines or bulleted layout for the export to destroy.** **This is precisely how `RT-09` passed on 2026-07-28 straight over `D-015`/`GAP-41`** — a live defect that discarded every line break in every exported answer — because "no corrupted or missing content" is satisfied by single-paragraph answers. The defect was found six days later by WJ reading a `.docx` his wife had downloaded after a real application, not by any test. ✅ **The fix itself is not in doubt:** `GAP-41` was fixed 2026-08-06 (one `TextRun` per line with `break: 1`), automated coverage unzips a generated `.docx` and counts `<w:br/>`, and WJ confirmed it manually in Word on 2026-08-14. **What is missing is confirmation that it holds on production**, which is a different claim from the one this run can support. **Owed: one export from a genuinely multi-line, multi-paragraph answer against production.** WJ noted the omission unprompted at the time and will cover it in the next flagship run (`MK-Community-Foundation-test-plan.md`). **Recorded as a caveat rather than a clean Pass so it cannot be quietly forgotten** — the last time this check was assumed satisfied, it hid a High defect for six days. ✅ **Discharged the same day by `MK-Community-Foundation-test-plan.md`'s `MKCF-09`**, run on production with genuinely multi-line answers: line breaks and blank lines survive into Word. The two flagships share the same export route, so that run answers this one. **`ABC-10` is a clean Pass.**
