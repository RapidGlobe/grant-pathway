...
| **B — Tell them what the document must contain** | The prep checklist names D5 explicitly and lists the funder's own required contents and length. **No AI, no writing surface, no export change.** Turns "gather these" into "here is what this one must cover". | **Small** | **Low**        |
...
B addresses the actual complaint — that the product short-changes the charity by staying silent — for a fraction of the cost. The extraction already captures `supportingDocuments`; this extends what is captured and shown. It touches no schema, no export, and no AI budget.
...
- **Whether D5 already appears in the `supportingDocuments` checklist today.** The extraction captures document _categories_; D5 may or may not be among them for ABCT. **One look at Step 4 on the current production run answers this**, and it changes B's size — possibly to nearly nothing.
