...
- Canvas-based rendering (not `<iframe>`/`<object>`) is required specifically because highlighting a cited region is a hard requirement of the design (`PDR-DH-004`); a plain embedded viewer can display a document but cannot programmatically scroll to and highlight an arbitrary page/section from outside the embedded content.
- Curator-verified citations (P6.5) follow the same reliability-over-raw-coverage principle `ADR-DATA-006` already established for the rest of the item graph — a human confirms a citation once per funder rather than the product silently trusting an unsupervised per-application guess that could be subtly wrong on any given run.
...
3. **`ADR-SEC-004`** (HTTP security headers): the CSP needs `worker-src 'self' blob:` added for the canvas-based PDF rendering library's web worker (`next.config.ts`) — `connect-src` already covers the Supabase Storage fetch, no other directive change needed.
4. **`ADR-OPS-006`** (accessibility testing): three manual accessibility checklist items are added once the P6.4 viewer is built — keyboard navigation into/through the viewer, a screen-reader alternative for the canvas-rendered element, and focus management on open/close.
