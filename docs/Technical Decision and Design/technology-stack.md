...
**Rationale:**
Upstash provides a serverless Redis instance used for rate limiting. A sliding window algorithm limits requests per time window, providing a defence-in-depth layer on top of the monthly AI usage cap enforced at the database level. **The two work at different timescales and neither substitutes for the other:** Upstash stops a burst in the next sixty seconds, the database cap stops a month's overspend.

**There are two limiters and four call sites** (`lib/rate-limit.ts`, verified against the code 2026-08-16):

| Limiter           | Rule                           | Applied at                                                                                                   |
| ----------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| `aiRatelimit`     | 5 per 60s, keyed by user id    | `/api/generate-summary`, `/api/refine-answer`, **and the Charity Commission lookup in `actions/charity.ts`** |
| `resendRatelimit` | 3 per hour, keyed by **email** | `resendVerificationEmail` in `actions/auth.ts`                                                               |

⚠️ **The Charity Commission lookup shares `aiRatelimit` despite not being an AI call.** Defensible — both are calls to a metered external service — but the limiter's name does not say so, and a reader checking "what consumes a user's AI burst allowance" from the name alone would get it wrong. Recorded here rather than renamed, since renaming touches four files for no behavioural gain.

**Cron routes are deliberately excluded** from both limiters and must stay that way — see the comment in `app/api/cron/cleanup-guidelines/route.ts`. They authenticate with `CRON_SECRET` instead.

⚠️ **This entry has now been corrected twice for the same class of error — miscounting where the limiter is applied.** _2026-07-30 (audit finding **L2**): it read "all three AI API routes… and the draft generation route"; `/api/generate-draft` had been deleted on 2026-07-01, so the count was three when it was two._ _2026-08-16: it then read "**both** AI API routes", which undercounted in the other direction — `actions/charity.ts` had been using `aiRatelimit` without ever appearing here, and the `resendRatelimit` limiter was absent from this entry entirely._ **The table above replaces the prose count**, so a future change adds or removes a row rather than requiring the sentence to be re-counted. `security-review-2026-08-15.md` §-rate-limiting already recorded all three `aiRatelimit` sites correctly, which is how the undercount was spotted.
...
| Upstash account   | ✅ Active | **One** Redis database, `eu-west-1` (Ireland), free tier — **shared by dev and production**; see TS-07 |
...
| ------- | ---------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.14    | 2026-08-16 | Rapidglobe Ltd | **TS-07 corrected for the second time, and restructured so it cannot drift the same way again.** The entry read "**both** AI API routes — `/api/generate-summary` and `/api/refine-answer`". That undercounted: **`actions/charity.ts`'s Charity Commission lookup also uses `aiRatelimit`** and had never appeared here, and the second limiter — **`resendRatelimit`, 3 per hour keyed by email address** — was absent from the entry entirely. ⚠️ **Same class of error as version 1.11's correction, in the opposite direction:** that one fixed an overcount of three routes when there were two; this fixes an undercount of two when there are four call sites across two limiters. **A prose count in a document that is not the source of truth will keep going stale**, so the count is now a table — a future change adds or removes a row instead of requiring a sentence to be re-counted. Two facts recorded that were nowhere in this document: the Charity Commission lookup **shares the AI limiter despite not being an AI call** (defensible, since both are metered external services, but the name does not say so), and **cron routes are deliberately excluded** from both limiters. The service status table also corrected — **one** Redis database in `eu-west-1` (Ireland), free tier, **shared by dev and production** (`GAP-105` section G, 2026-08-16), where it previously read "rate limiting live on all AI routes". Spotted because `security-review-2026-08-15.md` had already recorded all three `aiRatelimit` sites correctly, so two project documents disagreed. |
