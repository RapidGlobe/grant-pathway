...
| 2026-07-16 | Added Consequences note: live-testing Clothworkers' Foundation found the plain character-ceiling truncation was dropping the actual application form's real questions in favour of front-loaded guidance prose. `lib/preprocess-text.ts` now detects a form-start heading and prioritises it in the truncation budget — see `CHANGELOG.md`'s 2026-07-16 entry for the full investigation and a second, separate bug found but not yet fixed (repeated-line stripping removing a genuine recurring question). |
