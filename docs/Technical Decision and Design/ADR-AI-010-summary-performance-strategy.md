...
- **Added 2026-07-16:** Live-testing Clothworkers' Foundation (a scheduled test funder for this ADR, see the original Context table) confirmed a real consequence of "truncate at a configurable character ceiling" that this ADR did not anticipate: a plain first-N-characters cut systematically favours front-loaded guidance/eligibility prose over a funder's actual sample application form when the two are combined in one PDF, silently dropping the form's real narrative questions rather than just the boilerplate this ADR intended to trim. `lib/preprocess-text.ts` now detects a form-start heading (`findFormStartIndex()`) and, when found, splits the ceiling budget between a capped preamble and the form section — see `docs/Implementation Plan/CHANGELOG.md`'s 2026-07-16 entry for the full investigation, including a second, separate bug found in the same investigation (the repeated-line header/footer stripping removing a genuine question that recurs verbatim across Clothworkers' three forms). Per this ADR's existing Consequences requirement, any further pre-processing change must be re-tested against all scheduled funders (Clothworkers', Henry Smith, Wolfson, Idlewild, A B Charitable Trust) before being trusted as a general fix, not just validated against the one funder that surfaced it.
- **Added 2026-07-16 (same day, fast-follow):** the second bug flagged above was fixed immediately at WJ's request rather than deferred, since it deletes real content regardless of truncation (confirmed reproducible with no ceiling applied at all). `detectRepeatedLines()`'s "identical 3+ times" rule for stripping running headers/footers could not distinguish a true header (repeats near-identically on almost every page) from a genuine question a funder happens to repeat verbatim across multiple forms. Fixed by requiring every occurrence of a repeated line to sit immediately adjacent to a `[PAGE N]`/`[SECTION: ...]` marker (`isAdjacentToMarker()`) before it's treated as boilerplate — a line with even one mid-page occurrence is kept everywhere. Spot-checked (cleaning-step only, no live Bedrock call) against 7 other scheduled-funder PDFs per this ADR's cross-funder testing requirement; full end-to-end re-verification per funder remains WJ's live-testing responsibility as usual.
...
| 2026-07-16 | Added Consequences note: live-testing Clothworkers' Foundation found the plain character-ceiling truncation was dropping the actual application form's real questions in favour of front-loaded guidance prose. `lib/preprocess-text.ts` now detects a form-start heading and prioritises it in the truncation budget — see `CHANGELOG.md`'s 2026-07-16 entry for the full investigation and a second, separate bug found in the same investigation (repeated-line stripping removing a genuine recurring question). |
| 2026-07-16 | Added Consequences note (same day, fast-follow): the repeated-line stripping bug flagged above was fixed immediately at WJ's request. `detectRepeatedLines()` now requires every occurrence of a repeated line to be marker-adjacent (a true running header/footer) before stripping it, not just identical 3+ times — see `CHANGELOG.md`'s matching entry.                                                                                                                                                          |
