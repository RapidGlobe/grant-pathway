...
`components/contextual-tooltip.tsx` is a reusable wrapper around `components/ui/tooltip.tsx` (Base UI), supporting 5 trigger variants (`focus`, `page-load`, `hover-disabled`, `first-click`, `persistent`) without any coachmark/tour library dependency. Dismissed-state is fetched server-side once per hosting page (`actions/tooltips.ts`'s `getDismissedTooltipIds`) and prop-drilled one hop into the relevant Client Component — the same pattern already used for every other piece of per-request state in this app — rather than fetched client-side on mount, which would flash `page-load` tooltips open before the dismissed-check resolved. Dismissal is recorded via `dismissTooltip`, an idempotent upsert into `user_tooltip_dismissals` (§5a, `docs/data-model.md`).
