...
│   ├── action-error.ts              # Server Action transport-failure messages (M8)
...
│   ├── governance-items.ts          # Governance/reserves item-graph helpers (P6.1)
│   ├── guideline-citations.ts       # Citation parsing and labelling (ADR-DATA-007)
│   ├── guidelines-session.ts        # sessionStorage get/set/clear for guidelines
│   ├── help-centre.ts               # Help centre article registry (PDR-UI-008)
│   ├── inactivity.ts                # Retention date arithmetic + cron eligibility (GAP-31)
│   ├── preprocess-text.ts           # Text normalisation before AI processing
│   ├── rate-limit.ts                # Upstash rate-limit helpers
│   ├── site-url.ts                  # Canonical public base URL + ALLOW_INDEXING (M5)
│   ├── structural-markers.ts        # [PAGE N]/[SECTION]/[ITEM N] marker regexes
│   ├── types.ts                     # Shared TypeScript types (AiSummaryData etc.)
│   ├── utils.ts                     # General utilities
│   ├── validation.ts                # Shared Zod schemas for Server Actions (GAP-25)
│   ├── version.ts                   # Build/version identifier
│   └── supabase/
...
**Two modules added to the `lib/` tree above on 2026-08-05** by the `P5.0` reconciliation (register ref **R-22**, `docs/Implementation Plan/pre-launch-reconciliation-2026-08-05.md`). Both were built on **2026-07-29** — the same day this document was last updated — and neither reached it, in a Tier 1 document whose update trigger is "any change to system architecture, data model, API contracts, or component design":

- **`lib/site-url.ts`** — exports `SITE_URL` (from `NEXT_PUBLIC_SITE_URL`, defaulting to the canonical domain, trailing slash stripped, truthiness-checked rather than `??` because `vercel env pull` writes empty strings) and `ALLOW_INDEXING` (from `NEXT_PUBLIC_ALLOW_INDEXING`, **opt-in** — `robots.ts` disallows everything unless the value is exactly `'true'`). Consumed by `sitemap.ts`, `robots.ts`, `layout.tsx`'s `metadataBase` and OG `url`, and all three transactional emails. Created under audit finding **M5**, after the three emails were found to be linking users to a registrar parking page. Both variables have `P5.6` go-live actions attached.
- **`lib/action-error.ts`** — the two user-facing messages for a Server Action that fails at **transport** level and returns no result at all (version skew from a deploy landing on an open tab, or a session expiring so React receives unparseable HTML). Distinct from the `{ok, error}` object contract used for expected errors. Created under audit finding **M8**, after Sentry `GRANT-PATHWAY-6` showed 8 such failures over three weeks, **88% of them on Step 4**, with nothing shown to the user. Backs Step 4's sticky `role="alert"` "Not saved." banner.
...
- Activity tracking monitors `mousemove`, `keydown`, `click`, and `touchstart` events
- A warning modal appears at 55 minutes of inactivity with an **"I'm still here"** option (extends the session) and a **"Sign out now"** option (ends it immediately)
- **Fixed 2026-07-28 (D-013):** once the warning modal is open, ambient activity from the events above is ignored — it no longer resets the timer or dismisses the modal. Previously, moving the mouse toward the modal's own buttons triggered the same document-level listener, closing the modal before it could be clicked; only the modal's own two buttons can now end the warning state
- If the modal is dismissed via "Sign out now," or ignored until the 60-minute mark, the user is signed out and redirected to `/?timeout=true` with the message: _"You've been signed out due to inactivity. Your work has been saved — sign in again to carry on."_
- **Built 2026-08-07 (`GAP-22`).** The redirect had been to a bare `/` since the timer shipped, so the sign-in page had no way to know why the user had arrived and the message specified above could never appear — the user was simply dumped on the sign-in screen. `doSignOut()` now appends `?timeout=true`; `app/(public)/page.tsx` reads it and passes `signedOutForInactivity` to `SignInForm`. This follows the `?deleted=true` pattern already on the same page. **Both sign-out paths are covered on purpose:** "Sign out now" and the 60-minute expiry both run through `doSignOut()`, which is what this bullet has always specified. The ordinary nav sign-out (`→ /`) and account deletion (`→ /?deleted=true`) are unaffected. Rendered as an amber notice with `role="status"`, not a red error with `role="alert"` — nothing has gone wrong and nothing was lost
- Auto-save (Step 4) ensures in-progress answers are saved before the timeout fires — which is why the banner can promise the user their work is safe
...
All data is retained for the lifetime of the user account. Account deletion removes the user's `guidelines-temp` Storage objects first, then cascades through all tables in order: `application_items`, `application_guidelines` → `applications` → `charity_profiles` → `ai_usage_log` → `user_profiles` → Supabase Auth user. Deletion is permanent and immediate. (ADR-DATA-003)

**Storage step added 2026-08-06 (`GAP-47`).** `PDR-DH-002` has always listed uploaded guideline files among the data deleted, but until this date neither deletion path touched Storage — they were left to the `cleanup-guidelines` cron, which sweeps the bucket hourly. Both paths now call `deleteUserGuidelineFiles()` (`lib/storage-guidelines.ts`) before the table cascade, so that nothing is yet destroyed if the Storage call fails. The step is ordered first for that reason, and its failure is reported to Sentry but does not fail the deletion: the cron remains the backstop, and blocking an erasure request on a transient Storage error would make a defence-in-depth measure a new single point of failure. See `data-model.md` §6 for why the mismatch was closed in the code's favour rather than by amending the PDR, and for the flat `{user_id}_{timestamp}` object naming that any code touching this bucket must use.
...
| `/api/cron/inactivity-warning`  | GET    | Send inactivity warning emails — once per inactivity cycle, guarded by `user_profiles.last_inactivity_warned_at`                                                                                           | Cron: 08:00 UTC daily |
| `/api/cron/inactivity-deletion` | GET    | Delete inactive accounts ≥24 months. Aborts with 503 before deleting anything if `RESEND_API_KEY` is unset                                                                                                 | Cron: 09:00 UTC daily |
...
**Repeat-safety of scheduled jobs (added 2026-08-05, GAP-31).** A cron whose eligibility test is a _range_ rather than a _threshold_ will match the same row on every run for as long as that range holds, so it must record what it has already done. `inactivity-warning` selects users who signed in between 23 and 24 months ago — a month-wide range, checked daily — and before this fix emailed each of them an account-deletion warning around thirty times. The guard is `user_profiles.last_inactivity_warned_at`, compared against `auth.users.last_sign_in_at` so it needs no reset step (see `data-model.md` §6).

`inactivity-deletion` needs no such guard — its test is a threshold and deleting a deleted user is a no-op — but it now **aborts with 503 before deleting anything** if `RESEND_API_KEY` is unset, mirroring `/api/account/delete`'s existing preflight. Without it, `lib/emails/send.ts` returns silently rather than throwing in that configuration, so the route's `try/catch` never fired and every account it deleted would have been deleted with no notification and no error raised. Email send failures in both crons, and in `/api/account/delete`, now reach Sentry tagged by `route` and `step` (ADR-OPS-005).

The date arithmetic and both eligibility tests live in `lib/inactivity.ts` rather than being duplicated in each route, so a change cannot move one boundary without the other — the two crons run an hour apart against the same user list, and no user may be claimed by both.
...
**Built 2026-08-07 (`GAP-21`), extended 2026-08-14 (`GAP-92`).** **Five** call sites: `generate-summary` tags its primary Bedrock call, its JSON-reprompt retry, and — added the same day by `GAP-52` — a **response truncated at `max_tokens`**, which is a failure path with no caught exception to classify and so has to raise its own error to report; `refine-answer` tags its single Bedrock-error catch, plus its own **response truncated at `max_tokens`** tag, added by `GAP-92` on the same pattern. `generate-summary`'s truncation tag uses `ai_error: 'response_too_long'`; `refine-answer`'s uses the distinct `ai_error: 'answer_too_long'` (see `AiErrorCode` in `lib/ai-error-handler.ts`) — both deliberately distinct from `parse_error`: all three arrive as unparseable JSON, but a parse error may clear on a retry and a truncation never will, so a wave of one means something very different from a wave of the other.

Two deviations from the original specification, both deliberate:

- **`captureException(err, { tags })` rather than `Sentry.withScope(scope => scope.setTag(…))`**, which is what this section showed until today. The two are functionally identical for tagging. The chosen form matches the six Sentry call sites that already existed in `app/api/account/delete/route.ts` and `app/api/cron/inactivity-deletion/route.ts`, so the codebase has one Sentry idiom rather than two.
- **An `ai_error` tag was added.** It carries the `classifyBedrockError()` code, because a wave of `throttled` and a wave of `unavailable` are different incidents and the `route` tag alone cannot tell them apart.

**Not yet verified end-to-end.** Unit tests assert every Bedrock failure path is tagged — including a scan that fails if a third Bedrock route is added untagged — but only the Sentry dashboard can show that a tagged event arrives, which is `P5.4`'s work.
...
| ------- | ---------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1.29    | 2026-08-14 | Rapidglobe Ltd | **`GAP-92` built — `refine-answer`'s truncation guard now matches `generate-summary`'s (`GAP-52`), and §14's Sentry AI-error tagging call-site count updated from four to five.** `app/api/refine-answer/route.ts` now logs input/output tokens split with `stop_reason`, checks `stop_reason === 'max_tokens'` before parsing, and returns a new `answer_too_long` `AiErrorCode` (`lib/ai-error-handler.ts`) instead of falling into `parse_error` — `answer_too_long` is deliberately distinct from `generate-summary`'s `response_too_long`, since the two routes' user-facing messages must say different things. §14's tagging paragraph updated to describe the new fifth call site (the route's own truncation tag, `ai_error: 'answer_too_long'`). Full detail in `docs/Implementation Plan/ADR-TRACEABILITY.md` v2.71.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 1.28    | 2026-08-07 | Rapidglobe Ltd | **§5 and §14 both described behaviour the code did not have; both are now built, and §14 was corrected to the form actually implemented.** §5 has said since 2026-04-21 that an inactivity sign-out lands on the sign-in page with "You've been signed out due to inactivity" — the redirect was to a bare `/`, so the page had no way to know why the user had arrived and the message had no mechanism to appear. Now `/?timeout=true` (`GAP-22`), reusing the `?deleted=true` pattern already on that page; the bullet records that both sign-out paths show it, as §5 always specified, and that the banner is an amber `role="status"` rather than a red `role="alert"` because nothing has gone wrong and auto-save has already saved the work. §14's AI route tagging (`GAP-21`) was likewise never implemented — **neither AI route imported Sentry at all**, so every Bedrock failure had reached the dashboard untagged since the routes were written. **§14's code sample has been replaced rather than annotated:** it showed `Sentry.withScope(scope => scope.setTag(…))`, and the build uses `captureException(err, { tags })`, which is identical for tagging and matches the six Sentry call sites already in the codebase. Leaving the sample as written would have left this document describing something the code does not do, which is the same condition that let both gaps sit unnoticed for three months. The extra `ai_error` tag is documented as a deliberate addition beyond the spec, and §14 now states plainly that the tagging is **not** verified end-to-end — unit tests prove the call is made, only the Sentry dashboard can prove an event arrives, and that is `P5.4`. |
| 1.27    | 2026-08-06 | Rapidglobe Ltd | **GAP-47 built — the Data retention section's deletion cascade now begins with the user's `guidelines-temp` Storage objects.** `PDR-DH-002` had listed uploaded guideline files among the data deleted since 2026-04-16 while neither deletion path touched Storage; both now call `deleteUserGuidelineFiles()` (`lib/storage-guidelines.ts`) ahead of the table cascade, deliberately ordered first so that nothing is yet destroyed if the Storage call fails, with failure reported to Sentry but not failing the deletion — `cleanup-guidelines` remains the backstop and a defence-in-depth step must not become a new single point of failure. Recorded here with a pointer to `data-model.md` §6 for the reasoning and for the flat `{user_id}_{timestamp}` object naming, since the folder path this and other documents implied has never existed (`GAP-48`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 1.26    | 2026-08-05 | Rapidglobe Ltd | **GAP-31 built — new "Repeat-safety of scheduled jobs" note under the cron section (§ Scheduled Jobs), plus route-table detail for both inactivity crons.** Records the general rule the bug came from: a cron whose eligibility test is a _range_ rather than a _threshold_ re-matches the same row on every run and must record what it has already done. `inactivity-warning`'s range is a month wide and it runs daily, so it emailed each affected user the same account-deletion warning around thirty times — not the double-fire edge case the 2026-06-08 audit described. Also records `inactivity-deletion`'s new `RESEND_API_KEY` preflight and why the previous `try/catch` could never have caught that case, and the move of both eligibility tests into `lib/inactivity.ts`. **Seven `lib/` modules were missing from the §4 project-structure tree** and are now listed: `governance-items.ts`, `guideline-citations.ts`, `help-centre.ts`, `inactivity.ts`, `structural-markers.ts`, `validation.ts`, `version.ts` — `validation.ts` was added the previous day under GAP-25 and shows the same drift `1.25` fixed for two other modules, so the tree needs checking whenever a `lib/` module is added, not only when the architecture changes.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 1.25    | 2026-08-05 | Rapidglobe Ltd | **`P5.0` requirements reconciliation** (register: `docs/Implementation Plan/pre-launch-reconciliation-2026-08-05.md`). **`lib/site-url.ts` and `lib/action-error.ts` added to the Section 4 project-structure tree, with a note on what each does and why** (register ref **R-22**) — both were built on 2026-07-29 under audit findings **M5** and **M8**, the same day this document was last updated, and neither reached it. **Also found and recorded: this document was stamped `Version: 1.24` with no 1.24 row in this table** (register ref **R-23**) — the 2026-07-29 change bumped the version and last-updated date without recording what changed, so there is no way to tell from the document what 1.24 was. Not reconstructed retrospectively; the gap is stated rather than invented. **Delta check against Phase 6 came back otherwise clean** — the item graph (`application_items`), `application_guidelines`, citations (`source_type`, `heading_path`) and `first_exported_at` are all present and correct, consistent with this being an actively maintained document rather than an unswept one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 1.24    | 2026-07-29 | Rapidglobe Ltd | _No contemporaneous entry was written — see the 1.25 row above (register ref **R-23**). The version and last-updated date were bumped on this date without a record of the change._                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 1.23    | 2026-07-28 | Rapidglobe Ltd | Session timeout section corrected (found via documentation freshness audit): added `touchstart` to the tracked activity events (was missing); button labels corrected from an invented "Stay signed in" to the real "I'm still here"/"Sign out now"; documented the D-013 fix — ambient activity no longer dismisses the warning modal once it's open, only its own two buttons can (previously moving the mouse toward the modal closed it before it could be clicked).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
