...
| 2026-07-29 | **M7's headline count corrected — it was wrong.** The finding claimed twenty open Dependabot PRs with several superseded. There are **five**, none superseded; `dependabot.yml` caps each ecosystem at five, so twenty was never possible, and the superseded PRs described had already been closed on 24–27 July. The `node_modules`-vs-lockfile drift in the same finding was accurate. The real problem was daily updates with no grouping keeping the queue permanently full.                  |
...
| 2026-07-29 | **O11 and O12 added** — both read straight out of a Vercel build log while diagnosing why the TypeScript 7 Dependabot preview failed, so both are evidence-led. Sentry source maps are not uploaded (production stack traces are minified) and `onRouterTransitionStart` is not exported, so client-side navigations are uninstrumented. Both weaken the Sentry diagnostics that S4 and M8 depend on, and neither is covered by P5.4's Sentry checklist, which asks only for the DSN. Added to §6. |
...
> **Correction, 2026-07-29 — the headline count in this finding was wrong.** It originally read: _"Twenty Dependabot PRs are open on `origin`, several already superseded — `next` 16.2.9 **and** 16.2.10 are both open while 16.2.11 is what the lockfile pins; `lucide-react` has three open bumps (1.16, 1.17, 1.23) against 1.23 already in `package.json`."_ Checked properly during the walkthrough: **five** Dependabot PRs are open (#79, #81, #82, #84, #85), and none of them is superseded — each is a distinct package at its current latest version. `.github/dependabot.yml` sets `open-pull-requests-limit: 5` per ecosystem, so a twenty-PR npm backlog was never possible. The superseded PRs the finding described were real but had already been closed on 24–27 July, before the audit: #83 (`eslint-config-next` 16.2.11), #80 (`react-dom`), #72 (`eslint-config-next` 16.2.10), #70 (`eslint` 10.7.0, itself superseded by #79). Dependabot closes its own PRs when it opens a newer bump for the same package. The `node_modules` drift below was accurate.

The queue nonetheless had a real problem, just not the one described: `daily` updates with **no grouping** kept it pinned at its five-PR limit with one package per PR, so it stopped functioning as a signal — the same desensitisation pattern as the permanently-red `audit` job in **M2**.
...
**How it went unnoticed is the more useful part of this finding.** Three things aligned: (1) Next 16 entered the project on 2026-05-20 via commit `b369a95`, titled "fix: update dependencies to resolve security vulnerabilities" — a major framework upgrade arriving inside a security patch, so the release notes announcing the rename were never read; (2) the deprecation produces no lint error and no CI signal — **correction, 2026-07-29 (later same day): it does produce a build warning**, `⚠ The "middleware" file convention is deprecated. Please use "proxy" instead.`, visible in every Vercel build log since Next 16 landed. My original claim that the deprecation was silent was wrong. The accurate statement is worse for us, not better: the warning has been there for two months and nobody reads Vercel build logs on a green deployment; (3) `AGENTS.md` §1, the check written specifically to catch this class of change, was pointing at paths that did not exist (**M1**), so no session ever opened the Proxy page. M1 and O10 are the same root failure surfacing two months apart. With M1 fixed, the next session working in that file will see the deprecation notice.

**O11 — Sentry source maps are not uploaded, so production stack traces are minified.** _Added 2026-07-29, read out of a Vercel build log._

Every production build logs:
...
`SENTRY_AUTH_TOKEN` is not set in the Vercel environment. Two consequences, both of which degrade the diagnostics the rest of this audit depends on:

1. **Stack traces point at minified bundle positions**, not source files and line numbers. `GRANT-PATHWAY-6` — the issue that produced **M8** — was diagnosable only because its breadcrumbs happened to carry the URL and timing. A fault whose signature lives in the stack rather than the breadcrumbs would be far harder to place.
2. **No Sentry releases are created**, so errors cannot be attributed to a deployment. That is precisely the correlation M8's version-skew diagnosis needed, and it had to be reconstructed by hand from `git log` timestamps instead.

**Why it belongs with P5.4, not here:** P5.4's Sentry line currently asks only that the DSN is set and PII scrubbing is active. It does not ask for an auth token, a release, or source maps — so a P5.4 sign-off today would leave production with minified traces and no release attribution. This is one of the additions the Phase 5 restructure proposal recommends (`docs/Implementation Plan/phase-5-restructure-proposal-2026-07-29.md` §2.5).

**Fix:** generate a Sentry organisation auth token with `project:releases` scope, add it to Vercel as `SENTRY_AUTH_TOKEN` (Sensitive), redeploy. No code change — `next.config.ts` already wires the Sentry build plugin; it is only missing the credential.

**O12 — Client-side navigations are not instrumented.** _Added 2026-07-29, same build log._

```
[@sentry/nextjs] ACTION REQUIRED: To instrument navigations, the Sentry SDK requires you to export an
`onRouterTransitionStart` hook from your `instrumentation-client.(js|ts)` file.
```

Without it, App Router client-side navigations produce no Sentry transaction. For this product that matters more than it would for most: the five-step flow is almost entirely client-side navigation between `step/1` through `step/5`, so the journey users actually take is the part with the least observability. It also means an error thrown during a step transition may not be attributed to the step the user was on.

**Fix:** one line in `instrumentation-client.ts` — `export const onRouterTransitionStart = Sentry.captureRouterTransitionStart`. Small enough to fold into the P5.4 Sentry work alongside **O11**.

**Both O11 and O12 have been sitting in every build log since the SDK was installed.** That is the same pattern as **O10** above and worth naming as a standing risk in its own right: this project has no habit of reading build output on a successful deployment, so anything that warns rather than fails goes unseen indefinitely. Three findings in this audit now share that single cause.
