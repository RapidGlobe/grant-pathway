...
**Amendment, 2026-07-24 (same day, follow-up):** A 4th layer was added after this decision was first written — **UI / accessibility** (`help-and-tooltips-test-plan.md`), covering `PDR-UI-008`'s help centre link and contextual tooltips. This is a horizontal concern that touches many routes at once rather than varying by guideline shape or funder, so it did not fit cleanly into any of the three layers above; rather than force it into the flagship plans (which would blur their funder-flow-specific purpose) it gets its own layer, following the same reasoning that separated eligibility and guideline-shape out originally. See `TEST-DASHBOARD.md` v2.1.
