...
- "citation" (on each question and each section): the guidelines text below contains structural markers — lines reading "[PAGE N]" (PDF page boundaries) or "[SECTION: A > B]" (heading boundaries, docx/pasted text). For each question/section, find the marker whose text block it was drawn from and report it: set "source_type" to "page" and "page_number" to that N (with "heading_path" set to null), OR set "source_type" to "heading" and "heading_path" to the exact array shown in that marker, e.g. ["A", "B"] (with "page_number" set to null) — never set both page_number and heading_path to non-null values. "quote" must be a short (under 20 words) verbatim excerpt copied exactly from within that marker's text block — something a person could search for and find. If you cannot identify a specific marker a question/section came from, set "citation" to null entirely — do not guess or invent a page number or heading path
