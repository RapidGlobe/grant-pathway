 ...
 *   - Set question_type to 'narrative', 'date' or 'number' (D-021, 2026-08-20).
 *     'narrative' is the default. This replaced the old narrative-only gate,
 *     which asked the model to decide whether a date or numeric question was
 *     worth extracting at all — and got both answers: the Radcliffe fixture
 *     extracted its project dates as prose writing cards (D-021 as raised),
 *     while the Idlewild fixture dropped its two mandatory date questions
 *     entirely. Same rule, opposite outcomes, because the schema had no way
 *     for the model to say "this is a date". It can now say so, so it no
 *     longer has to choose between misrendering and discarding.
 *   - question_type and is_budget_question are INDEPENDENT: a project cost
 *     figure is number + budget; a question about financial plans is
 *     narrative + budget
 *
 ...
 *
 * GROUPED BUDGET FIGURES (GAP-51, found by GCM-06's live re-run of the Stony
 * Stratford form, 2026-08-07). §5 "BUDGET FOR THIS PROJECT" b) asks for three
 * labelled money figures in a run — "Total needed for this project", "Amount
 * requested from SSTC", "Balance outstanding". Only the first became a card.
 * The other two are not optional extras: "amount requested" is the single
 * figure every grant decision turns on, and "balance outstanding" is the input
 * to §6's four narrative questions ("HOW WILL YOU RAISE ANY OUTSTANDING
 * BALANCE?"). An application exported without them is incomplete.
 *
 * Two things made this survive the 2026-07-27 fix above. First, "amount
 * requested" also appears as `AMOUNT REQUESTED £` in the form's front
 * administrative details table, which is correctly excluded — so the model
 * appears to have excluded the *name* rather than the *field*, and the
 * exclusion travelled to the budget section's own ask. Second, three labelled
 * currency lines under a heading look like a table summary, and everything
 * this prompt says about totals is about excluding them.
 *
 * The rule below therefore names both sides of the line rather than one:
 * grouped labelled money asks are each their own question; the arithmetic
 * footer of a costs table the applicant fills in themselves is not, because
 * the itemised breakdown is already a question and the footer sums its own
 * cells. WJ's decision, 2026-08-07 — project budget questions the funder has
 * actually asked are never excluded, which is what `lib/prompts.ts` has said
 * since 2026-07-27 and what step 7 of GCM-06 was misread as denying.
 *
 * Rule present, behaviour unverified — the same standing GAP-40 had, and for
 * the same reason: a prompt test can assert the words exist and can never say
 * what the model does with them. Closes only on a live re-run of GCM-07.
 *
