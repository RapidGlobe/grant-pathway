 ...
 *
 * Fixed 2026-07-17 (found live-testing Stony Stratford Town Council): a
 * section built from a tick-list instruction ("tick all that apply") plus a
 * separate narrative follow-up on the same topic ("please outline how...")
 * was showing no citation badge at all, even though a real heading marker
 * existed. `validateCitation()` (lib/guideline-citations.ts) only checks that
 * the reported marker exists — it never inspects the quote's content — so
 * the missing badge meant the model was returning "citation": null itself,
 * most likely reading the tick-list/table portion as the "formatting
 * instruction" the rules already say never to quote from, and then treating
 * the whole combined item as too ambiguous to cite at all. Added an explicit
 * rule: quote from the narrative follow-up's own wording in this pattern —
 * it IS the item's own text, not an incidental detail. Not yet independently
 * verifiable against a real Bedrock call locally (dotenvx redacts AWS
 * credentials for this agent) — WJ's next guideline upload for this funder
 * is the outstanding live-verification step.
 *
